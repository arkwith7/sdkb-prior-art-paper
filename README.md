...
산출물 평가는 다섯 에피소드로 수행되었고, 각 판정의 정본은 [paper/verdicts.yaml](paper/verdicts.yaml)
...
| **EP5** | 이식 판정 | 형식 층과 교차 태스크 층이 자원을 바꾸어도 동일하게 작동하는가 (T1·T2 미이식) | 형식 층·T3 는 코드 변경 없이 실행 · 정상 델타 30건 위양성 0(95% 단측 상한 9.5%) · 판정 가능한 결함 12건에서 T3 단독 검출 0 · 릴리스 계보 10판정 중 부분 승인 2 |
