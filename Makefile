...
# **보충자료 편입 (2026-08-29 · 사용자 승인).** 검사군 다섯이 전부 supplementary 를 제외하고
# 있었고, 그 사각지대에서 두 세대 낡은 판정 서술과 §0.8 금지 문구가 살아남았다(편입 시점 실측
# 11건 · 전량 S5). 대상은 국문판뿐이며 감사 기록(paper/audit/)은 제외한다.
verdicts:
...
# 어체 규격의 대상이 아니다 — 앞의 것은 감사 기록이고, 뒤의 것은 앞선 판에서 옮겨 온 표·부록의
# 사본이라 어체를 고치면 원문과 어긋난다. 판정 강도와 링크는 verdicts·submission-check 가
# supplementary 까지 본다(2026-08-29).
# 보는 것은 아홉 — S3 문장 길이 · T2 구어·은유 · T3 문두 접속어 · T5 주장 볼드 · T6 표·캡션·
# 소제목 · T7 축약형 · H1 소제목 명사구형 · V5 용어 단일화 · V6 표·그림 연번 · V7 `E` 라벨
# 이름공간(적격심사를 `E` 로 부르지 않는다 · CLAUDE.md §0.9 규칙 5).
