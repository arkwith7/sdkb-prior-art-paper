...
| **상류 환류 (C0)** | `DEFECT-LEDGER.md`(결함 D-01~**D-37**) · **살아 있는 CR 8건** — **투고 게이트 2건**(**CR-015** 공개본 경계 · **CR-016** 공개 재현 경로 · 2026-08-09 발행 · D-36·D-37) + 투고 후 6건(CR-001A·002·003·005·006·009) · `archive/`(종료 6건 — CR-004R·007·008·**011**·**012**·013 · 재송부 금지) · `HANDOFF-QUEUE.md`(송부 순서 · **상류 대기 0**) | `upstream/` |
...
| 주장 | 논문 | 증거 산출물 | 상태 |
|---|---|---|---|
| **C0 상류 환류** | 논문 밖 (제품 목적) | [DEFECT-LEDGER](../upstream/DEFECT-LEDGER.md) D-01~**D-37** · CR 15건(종료 7 · 살아 있음 8 — 그중 **투고 게이트 2건** CR-015·CR-016) · HANDOFF-QUEUE | **상시** — **상류 대기 0 · 하류 미결 0(2026-08-08 9차)** · 막는 것 없음 · 다음은 개봉 사전등록 |
...
| **PLAN — 살아 있는 계획 (3건 · 2026-08-08 5차 갱신)** | `PLAN-047`(판독 B — **개봉·판정 완료** · 잔여는 C2′ B층 + 원고 반영) · `PLAN-033`(원고 재구성 — §6.2–6.4 재작성 미완 · **B층 결과가 들어왔으므로 이제 착수 가능**) · `PLAN-038`(C2′ 전달 — 잔여는 B층 유상 실행 1건 · 판정식·마진은 PLAN-047 §4 에 동결됨).<br>**2026-08-08 판독 B 완료로 `archive/` 이동: `PLAN-031`**(B층 수집·프로토콜 — 개봉 1회로 생애 종료) · **`PLAN-040`**(+ `PLAN-040-oprime-snapshot-signatures.json` · 판독 A·B 둘 다 완료).<br>**2026-08-08 9차 `archive/` 이동: `PLAN-045`**(B층 질의 하류 소비 — S1–S7 통과 · 종료) · **`PLAN-046`**(D-33 코퍼스 재조립 사전등록 — 예측 4/4 · 불변식 8/8 · 종료).<br>**2026-08-08 `archive/` 이동: `PLAN-018`**(실행 완료 — **F1–F18 동결은 여전히 현행 코드의 계약이다 · 인용은 `archive/` 경로로**) · **`PLAN-029`**(미승인 로드맵 · 실질을 PLAN-030/031/035/040/041/045 가 나눠 가짐 · **§3.2 질의 번역 · §3.3 질의 표현 비교는 아직 미승계**) |
