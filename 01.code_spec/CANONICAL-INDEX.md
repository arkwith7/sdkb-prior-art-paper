...
| **논문 그림·표** | **v0.9 체계 재정의 중 · 대부분 미산출**(C2/C3 미측정). 구 커버리지/시계열/이식성 그림·표는 `paper/archive/`(S-시리즈·인용 금지) | `viz/figures.py`(예정) |
...
| **C2 · 핵심증명** | 선행기술 검색에서 온톨로지 보강이 텍스트 기준선을 (조건부) 개선 | 기여 3 | 미측정 |
| **RQ1 / H1·H2** | 검증 게이트 (H1 게이트 판별력 · H2 승인 안전성) | C3 | 미측정 |
| **RQ2 / H3** | 검색 유용성 (하이브리드 효과 · 조건부 통합) | C2 | 미측정 |
| **RQ3 / H4·H5** | 계층 기여(H4) · 특이성 음성대조군(H5) | C2 | 미측정 |
...
| **C2 핵심증명** | §5–6 (RQ2·H3 · RQ3·H4·H5) | Recall@100·nDCG·MRR·ablation·부트스트랩·하위집단 | **미측정** (IR 하네스 구축 예정) |
...
- **미측정 = 결함 아님.** v0.9는 IR 하네스(Pyserini BM25 nori + FAISS flat Dense + RRF · Titan Embed v2 ·
  Haiku 4.5 질의번역)를 신규 구축한다. C2/C3 산출물은 그 뒤 채워진다 — 지금 비어 있는 것이 정상이다.
