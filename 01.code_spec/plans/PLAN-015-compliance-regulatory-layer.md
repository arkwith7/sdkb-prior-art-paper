...
*상태: **✅ 완료 (2026-07-15)** · 지지 대상: **S3 (구 RQ3/소부장 IP-R&D)** · 논문 §4.6 · §5.3*
...
## 5.1 인접 강화점 — KSIC 정규화 (REF-001 §6·§8 에서 도출)

IP-R&D 실무 정본([REF-001](../REF-001-ip-rnd-domain-framework.md))이 짚은 KIPRO 분류 연계
(`CPC ↔ KSIC ↔ HSK` = "산업·품목 → 특허분류 공식 경로")는 규제 축과 직결된다: 수출통제는
**품목(HSK)·기술분류로 판정**되므로, 회사의 `companyType`(현재 자유 문자열 18종)을 **KSIC 코드로
정규화**하면 규제↔회사↔특허가 공식 경로로 이어진다. RQ3(G₂)에서 함께 검토하되, 이 계획의 **본체는
아니다** — 규제 인스턴스 적재가 먼저다.
