...
- 검색·랭킹(retrieval)은 T-Box 밖이다. 개념 오버랩을 후보로 *제안*할 뿐, 정답 판정은
  `hasPriorArtExaminer` 대조로 분석층이 한다. **← 폐기(2026-08-22 · PLAN-069 · §6 개정 주석 참조)**
- **FTO 판정도 T-Box 밖이다.** 침해 여부의 법적 판단은 온톨로지가 하지 않는다 — 겹칠 *가능성 있는*
  한정요소·청구항을 가시화할 뿐이고, 청구항 단위 침해 판단은 변리사·분석층이 한다.
  **← 부분 폐기(2026-08-22 · PLAN-069).** *법적 최종 판정*을 하지 않는다는 부분은 유지한다
  (§0.4 실무 이익 3등급 — FTO 는 여전히 등급 3 이다). 폐기되는 것은 **"대비 자체를 하지
  않는다"** 는 부분이다 — 한정요소 포섭·등가는 법적 판정이 아니라 **기술적 대비**이며,
  그것을 T-Box 밖에 두었기 때문에 pa CQ 5개 중 선행기술 관계를 계산하는 것이 0개가 되었다.
...
> **개정 (2026-08-22 · PLAN-069 · 사용자 승인 · §2.2 요건 5).**
> **아래 "보장하지 않는 것"의 둘째 항목과 §3.7 의 대응 문장은 폐기한다 — 삭제하지 않고
> 폐기 사유와 함께 남긴다.** 취소선이 아니라 이 주석으로 표시하는 이유는, 그 비목표가
> **원고 §6 의 검색 수치를 낳은 설계 결정 그 자체**이기 때문이다. 지우면 왜 그런 수치가
> 나왔는지가 사라진다.
>
> **폐기 대상 셋** — ① *"응답의 옳음·매칭 랭킹 … T-Box 는 연결만 제공"* ② §3.7 *"검색·랭킹은
> T-Box 밖이다"* ③ §3.7 *"FTO 판정도 T-Box 밖이다 … 가시화할 뿐"*.
>
> **왜 폐기하는가 — 실측 넷(2026-08-22 · PLAN-069 §2 · 벤더 스냅샷 `0a7ff153` 기준.
> 원고 §6 의 측정 세대 `d578bf3` 은 디스크에 없어 재계수 불가 · §1-3 · PLAN-069 §7).**
> ⓐ T-Box 7파일 1,577 트리플에서 `subClassOf`·`domain`·`range` 를 넘는 OWL 공리는 **3개뿐**이다
> (`equivalentClass` 1 · `TransitiveProperty` 2 · `Restriction` 0 · `propertyChainAxiom` 0 ·
> `inverseOf` 0 · `disjointWith` 0 · SWRL 0). 상류도 같은 진단을 갖고 있다 —
> `upstream/DEFECT-LEDGER.md` §1.4 **D-03** *"위반할 공리가 하나도 없어서 어떤 그래프도
> 일관적이다"* · 실증은 결함주입 F12 **0/9 미검출 2회**.
> ⓑ 그런데 T-Box 는 `ont:ClaimFeature`·`ont:PriorArtJudgment` 를 **"중심축"** 이라 선언하고
> (`sdkb-patent.ttl:507`), `dependsOnClaim` 주석에 *"진보성(§29②) 판단의 added-feature 는 여기에
> 있다"* 라고 적으면서 그 계산을 **자기 밖으로 위임**한다.
> ⓒ **벤더 스냅샷**의 그 중심축 인스턴스는 **전부 0건**(`hasClaim`·`hasFeature`·
> `PriorArtJudgment`·`ClaimFeature`·`NoveltyScore`) — **상류 그래프에는 있다**
> (D-07 §1.13 *"승격은 됐고 전달이 안 됐다"*). 검색이 실제로 쓴 것은 일반 분류 어휘 6종의
> **비가중 Jaccard** 다.
> ⓓ 그 결과 §29①·§29② 는 `skos:notation` 문자열 둘로만 존재하며, 어떤 추론기도 *"29①은 동일성·
> 단일 문헌, 29②는 조합 가능"* 을 구별할 수 없다.
>
> **곧 이 비목표는 지켜졌고, 지켜진 결과가 태스크 도달 실패였다.** PLAN-069 는 이를 결함이
> 아니라 **설계 결정의 귀결**로 기록하고, 판단 공리를 T-Box 안에 넣는 방향으로 기조를 바꾼다.
> 새 공리 명세는 PLAN-069 §4 R1(X1–X5)이며, 구현은 상류 CR(§4 R2)로 나간다.
> **상류 CR-005 도 같은 비목표를 갖는다**(*"추론 기반 신규 관계 생성은 이번 범위 밖"* ·
> `upstream/CR-005-tbox-logical-axioms.md:48-49`) — 따라서 공리 CR 은 **CR-005 의 확장이 아니라
> 별건**이다.
>
> **이 개정은 판정을 하나도 바꾸지 않는다** — H1–H5 · EP1–EP5 record · DP 등급 · §6 의 모든
> 수치가 그대로다. 바뀐 것은 **앞으로 T-Box 가 무엇을 하기로 하는가**이다.
