...
> **v0.9 라벨 주의.** 이 용어집의 H1/H2/RQ1-3 표제어 중 커버리지·시계열·이식성 의미는 구 라벨(**S1/S2/S3**)이다. v0.9 확증 가설 H1–H5는 별개(검증게이트·검색유용성·계층기여). 기준: [RECONCILIATION-v09.md](RECONCILIATION-v09.md) §1.
...
> 특허 보강 코퍼스에만 있어 baseline 만 떨어진다 → 구 SPEC-003, 현행 CQ 계약은 SPEC-004)
...
| **개념 축** | H2 의 집계 단위 = **Process(49) ∪ Device(34)**. HBM·GAA 는 공정이 아니라 디바이스다 **[v0.9: → S2(구 시계열/조기탐지 H2)]** |
| **관측 단위** | H1 의 표본 단위 = **공정 49개**(Process 11 + SubProcess 38). 복원 이전 20개 집합으로도 병기 검정한다 **[v0.9: → S1(구 커버리지 H1). v0.9 확증가설 H1은 게이트 판별력으로 다른 개념 — RECONCILIATION-v09.md §1]** |
| **커버리지 C(s)** | 공정 단계 s 에 매핑된 특허의 존재/수. G₀ 는 49개 중 **20개 커버, 29개 공백**(C₀ 20/49) **[v0.9: → S1(구 커버리지 H1)]** |
