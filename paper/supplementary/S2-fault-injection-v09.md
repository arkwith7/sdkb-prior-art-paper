...
> 결함주입에 의한 게이트 판별력 평가는 **네 차례의 판정을 거쳤다.** 앞의 세 차례는 같은 결함
> 데이터에 대한 재판정이므로 탐색적이고, 마지막 한 차례만 확증이다. 판정 규칙을 동결한 채 아직
> 판정한 적 없는 홀드아웃 인스턴스로 복제하였으며 사전등록은 결과 커밋과 분리하였다. 각
> 차례에서 무엇을 동결하고 무엇을 바꾸었는지는 아래 각 절의 첫 문단에 있다. 세대별 역량질문
> 통과율 표를 함께 싣는다.
...
§6.5.1의 진단은 처방을 지목했고, 그 처방("CQ 세분화 후 재실행")은 결함 주입 **이전에** 사전등록되어 있었다. 이 절은 그 처방을 실제로 수행한 결과다. 판정 규칙은 결과를 보기 전에 동결했으며(PLAN-021), 규칙 자체는 다음과 같다.
...
**표의 판정 열은 코드가 채운다.** `graph_v1` 세대는 `--against g0`로 동결하여 판정이 아티팩트 안에 있으며, 판정 없는 비-기준 세대를 만나면 생성기가 자리표시자 대신 예외를 던진다. 이 계약은 회귀 테스트로 고정되어 있다(`tests/test_tgate.py::test_table_refuses_placeholder_verdict`).
