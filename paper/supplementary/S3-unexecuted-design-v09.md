# S3 · 설계하였으나 실행하지 않은 평가 옵션

## S3-A · 검증 강도와 일반화 가능성의 결손 여덟

> 본문 §6.4에서 이동하였다. [본문 §6.4로 돌아가기](../manuscript/stage3_source.md#64-한계--경쟁-설명--결손의-명세)
...
| ⑤ | 승인 게이트를 완전히 평가할 수 있는 자원의 부재 | 형식 층과 T3은 제2 자원에서 판정하였으나 전체 게이트 평가는 미완성이다 (§4.6 · §5.5) | 릴리스 계보와 하류 태스크 벤치마크를 함께 갖춘 자원에서 재평가 |
| ⑨ | 결함 명세의 자원 간 이전 | 포함관계 역전 결함 9건은 주입 후보가 없어 판정에 이르지 못하였다 (§5.5) | 대상 자원의 술어 방향을 실측하여 결함 명세를 재정의한 뒤 재판정 |
| ⑩ | 세 뷰 가운데 외부 정답을 가진 뷰가 하나 | 선행기술조사만 심사관 인용이라는 외부 기준을 가진다. 전문가 매칭 뷰의 사례는 온톨로지에서 생성한 합성 인스턴스이고 기술예측 뷰에는 태스크 인스턴스가 없으므로, 두 뷰는 역량질문 통과율로만 감시하였다 (§3.1 · §5.1) | 온톨로지 밖에서 오는 정답을 구축한 뒤 재판정. 합성 사례의 링크를 가리고 복구율을 재는 설계는 순환이므로 쓰지 않는다 |
| ⑪ | 교차 태스크 검출 수가 공리 구성에 조건부 | T3 단독 검출 12/45는 논리 공리가 사실상 없는 T-Box에서 관측한 값이다. 형식 층이 이를 검출할 수 없음은 실행 이전에 선언하였다 (§5.4) | 상류 요구 CR-005의 공리 집합을 덮은 반사실 그래프에 형식 층만 재적용하여 귀속의 이동을 측정 |

해소 방법은 모두 새 사전등록의 대상이다. 검색 층 평가에 국한된 결손 ⑥–⑧은 S9에 있다.
**결손 번호는 고정 식별자이며 재부여하지 않는다** — ⑩·⑪은 다음 번호를 새로 받은 것이고 기존
①–⑨의 지시 대상은 바뀌지 않았다.

> 본 연구가 설계 단계에서 명세하였으나 실행하지 않은 평가 옵션의 전문이다. 질의 표현의 비교,
> 신규성과 진보성을 가르는 보조지표, 전문가 관련성 판정 프로토콜이 여기에 속한다.
> **실행하지 않았다는 사실 자체가 보고 대상이므로 명세를 그대로 싣는다.** 이 파일은 판정이나
> 수치를 새로 만들지 않는다.
