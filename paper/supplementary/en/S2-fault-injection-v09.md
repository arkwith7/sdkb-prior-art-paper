...
refinement of the CQ decision, and this table is the first quantitative evidence for its expected effect.
...
The diagnosis of §6.5.1 identified a remedy, and that remedy had been preregistered ("re-run after
...
**The verdict column of the table is filled by code.** The `graph_v1` generation was frozen
with `--against g0` so that the verdict lives inside the artifact, and the generator raises an
exception rather than printing a placeholder when it meets a non-reference generation without a
verdict. This contract is pinned by a regression test
(`tests/test_tgate.py::test_table_refuses_placeholder_verdict`).
