...
- **지표 관례 두 가지가 원고 §5.1의 예고와 다르다.** qrel이 전량 등급 1이어서 nDCG@20은 **이진 이득**, bpref는 **retrieved-as-judged** 관례로 계산했다(§5.1·§6.2 고지). bpref 값은 **과거 실행 기록이며 현행 분석에서 제외한다**([S5](S5-submission-full-v2.md) §5.9 지위 고지). 등급형 평가는 §5.5 전문가 판정 확보가 선결 조건이다.
...
## 부록 · 본문 §3.2·§3.3에서 이관한 자원 명세 (PLAN-086 §7.3)

### 거절 특허 축의 술어 전량

거절 특허 축은 `hasPriorArtExaminer`(심사관 인용 · **누출 통제 시 검색 그래프에서 제거되는
관계**) · `rejectedFor`(거절근거) · `hasClaim`/`dependsOnClaim` · `hasFeature`/`featureConcept` ·
...
고유 특허의 수이다. 2,211은 노드 수준에서 그래프에 도달하는 정답의 수이고, 584는 판단 연결이 있는
표본의 수이다. 네 수는 서로 다른 분모이므로 하나의 정답 수로 혼용하지 않는다.

### 적합성 등급 둘의 판정 규칙

등급 2는 판단이 특정 청구항과 선행문헌을 연결하고 거절근거가 식별된 경우이다. 등급 1은 특허
수준의 인용 관계만 확인된 경우이며, 인용 관계가 없으면 음성으로 확정하지 않고 미관측으로
처리한다. 등급 2가 법적으로 더 깊은 관련성을 의미하지는 않는다. 비특허문헌 30건은 주 평가의
분모에서 제외하여 별도로 보고한다.

### 정답 유입을 차단하는 릴리스 다섯 구분

공개 가능한 코어, 개발·검증용 qrel, 평가 시점까지 봉인된 테스트 판단(해시 고정 · 접근 로그),
qrel과 독립적으로 생성한 파생 특징, 출처이다. 고정은 이후 버전의 개선을 제약하지 않는다.
