...
> **제목 변경 기록 (2026-08-12 · PLAN-048 3단계).** 앞선 제목은 *"자원 지표는 태스크 성능을 …"* 의
> **무조건 일반형**이어서 §0.8 문구 사전에 걸린다. 이 연구가 보인 것은 **반례 하나와 그에 대한 결정**
> 이므로 제목을 산출물·설계 중심으로 바꿨다. **판정·수치는 하나도 바뀌지 않았다.**
...
> **원고 상태: v2.0 재구성 4단계 (2026-08-09) · 작업 언어 = 한글.**
> 4단계가 더한 것은 셋이다 — **§6.4a**(두 번째 확증 분할의 재확증 · H3 미지지 · A8 재현 실패) ·
> **§6.8 전면 개정**(A층 탐색 + B층 확증 + T4 판정 1회 = **실패** · 채점기 수리 전후 병기) ·
> **§8.3 교체**("만들지 못한 산출물" → 산출물 + 부록 이관 · F7·F6 0건 명시). 그것이 바꾸는 다른
> 절의 진술을 맞췄다(초록 · §1.4a · §1.5 · §4.9.1 · §5.5 · §6.2 · §6.3 · §7.3 · §7.5 · §7.6 ·
> §8.1 · §8.2 · §9.5 · §10.2 · §11).
> **확증 가설 2개 · 기여 2개 · H1–H5 판정 · §6.2–6.4 수치는 하나도 바뀌지 않았다** — 새로
> 기입된 판정은 전부 **별도 사전등록(PLAN-047 동결 커밋 `67568c8`) 아래의 두 번째 판정**이며
> 앞의 판정을 소급 수정하지 않는다. **T4는 발효 요건을 채웠으나 승인식에 넣지 않는다.**
> 절차 대장은 `01.code_spec/archive/PLAN-033` §12(규약 §2.2 요건 다섯).
>
...
> §9.5 · §10.2 · §11). **T4 발효 조건 ①(1회 완주)이 채워졌고 ②(마진 동결)는 남았으므로 T4는
...
> 2단계에서 바꾼 것은 **무엇을 앞에 놓고 말하는가** 하나다 — 제목·초록·§1·§2.6·§7.3·§11을
> **층 사이의 어긋남**을 1축으로 다시 썼고, 그 증거(자원만 교체한 두 팔)를 §9 한계에서 **§6.7
> 결과**로 옮겼으며, §4.9.1에 **T4**(하류 생성 층 비회귀)를 설계로만 신설했다.
> **H1–H5의 판정, §6.2–6.4의 수치, 확증 가설 2개, 기여 2개는 하나도 바뀌지 않았다** —
> 사전 점검과 이동 대장은 `01.code_spec/archive/PLAN-039`.

> **원고 상태: v2.0 재구성 1단계 (2026-08-01).**
> 확증 가설을 **H3(하이브리드 효과)·H5(계층 특이성)** 둘로 좁히고, H1(게이트 판별력)·H2(승인
> 안전성)·H4(계층 기여)는 **판정을 바꾸지 않은 채 지위만 강등**했다(각각 §4.9·§6.5 · §9.1 ·
> §6.4). 가설 라벨은 사전등록의 이름을 그대로 쓴다 — 개수만 줄이고 번호는 다시 매기지 않았다.
> 재구성 계획과 이동 대장은 `01.code_spec/archive/PLAN-033`, 잘라낸 절의 전문은 `paper/supplementary/`.
> **영문 Abstract는 없다** — 영문화는 실험 결과로 논리 기조가 확정된 뒤 원고 전체를 한 번에 한다.
> 저장소에서 재현된 사실은 "관측 결과"로, 미수행 실험은 대괄호 미확정 표기로 구분한다.
...
> | 3 | §9.1 | H2는 v0.9에서 미검정이었으나 이후 **최초로 실검정되어 기각**됐다(새 사전등록) | D-19 해소 · D-23 |
> | 4 | §9.2 | 텍스트→개념 **매핑 단계의 부재** — 자유 텍스트 질의를 받을 수 없다 | D-14 |
> | 5 | §9.3 | 질의가 "이미 온톨로지에 등재된 특허"라는 **외적 타당도 제약** | D-14 |
> | 6 | §5.6 · §10.2 | 재현성의 두 구멍 — 스냅샷의 매핑 자산 누락 · 하이브리드 run 파일의 비결정성 | D-16 · D-22 |
>
...
> 읽혔다) · **⑨ §9.2** 후보 코퍼스의 한국어 문서 수 38,246 → **39,246**(세 언어 합이 40,552와
> 맞지 않던 오기 · 실행 출력 기준).
>
> **⑩ T4 주 지표의 분모 서술 정정 (2026-08-10 · PLAN-048 0단계).** §6.8.2가 인용 정확도(전체)를
> 주 지표로 고른 근거를 *"분모가 팔에 따라 움직이지 않기 때문"*이라 적었는데, **분모는 인용한
> 식별자 전량이라 팔마다 움직인다**(`rag/score.py` · A층 P1 2.2929 대 B3 2.4949). 같은 원고의 §5.1과
> §6.8.1 ①이 이미 반대로 적고 있었다. 실제 근거인 **"두 팔이 같은 질의 집합 위에 선다"**로
> 고치고, 마진 승계의 **표본 단위 차이**(T1 = 질의 단위 매크로 · T4 = 식별자 pooled)를 함께
> 명시했다. **주 지표·판정식·마진·판정 결과는 하나도 바뀌지 않았다** — 부정확했던 것은 선택의
> 근거 서술이며, 사전등록(PLAN-047 §4.1-1)도 소급 수정하지 않는다(§1-3). 같은 정정으로 §5.1의
> *"이 넷은 확증 지표가 아니며"*를 B층 확증 수행 이후 상태로 맞췄다(인용 정확도·환각률 = 확증 ·
> 나머지 둘 = 보고만).
...
**질문은 넷이고, 각 질문은 이미 수행된 분석으로 답한다 — 새 실험을 전제하지 않는다.**

| | 설계 질문 | 답하는 자리 |
|---|---|---|
| **DRQ1** | 세 태스크(전문가매칭·선행기술조사·기술예측)를 하나의 공유 T-Box 로 표현하면서 태스크별 확장이 가능한 온톨로지 데이터셋은 **어떻게 설계할 수 있는가?** | §3 · §6.1 |
| **DRQ2** | 형식적으로 유효한 온톨로지 변경이 하류 태스크 성능과 **다른 태스크의 기능**을 훼손하지 않도록 릴리스 전에 평가하는 게이트는 **어떻게 설계할 수 있는가?** | §4 · §6.5 · §6.6 · §6.7 |
| **DRQ3** | 자원 품질의 개선이 검색 성능과 실무 활용 가치로 전달되는 과정에서 **어떤 어긋남과 실패 패턴이 발생하는가?** | §6.3a · §6.7 · §6.8 · §7 |
| **DRQ4** | 온톨로지 신호는 **어떤 데이터·언어·검색 아키텍처 조건에서** 선행기술 검색에 기여하거나 기여하지 못하는가? | §6.2 · §6.4 · §7.2 |

**DRQ3 이 이 원고의 중심 질문이다.** 층마다 지표가 어긋난 관측이 세 번 나왔고(§7.5), 그 셋이
설계원리 DP1–DP6(§9)로 일반화된다.
...
> 싣는 자리를 바꿨을 뿐, 검정·임계·판정을 재산출하지 않았다. 아래는 **DRQ3·DRQ4 안에서
> 결과를 보기 전에 동결한 평가 점검**이며, 그 지위로 읽는다.
...
> 않았고**, 싣는 자리만 바뀌었다 — 각각 §4.9·§6.5, §9, §6.4의 절제 표다.
...
**RQ5. 전달(transfer) — 확증 가설(H)을 배정하지 않고 게이트 조건으로 판정한다.** 검색 층에서
관측된 이득은 한 층 아래 생성 층(찾은 문헌을 근거로 붙여 답을 만드는 단계)으로 옮겨가는가?
이 질문에는 **H 라벨을 새로 만들지 않았다** — 대신 §4.9.1의 **T4(하류 생성 층 비회귀)**가
판정 조건이 된다. 판독은 두 번이고 지위가 다르다. **첫 확증 분할(A층)의 판독은 탐색적**이며,
유의성 검정을 사전등록하지 않았고 이미 개봉한 분할 위에서 수행했다 — 하는 일은 계측기를 결과
보기 전에 동결하는 것이었다. **두 번째 확증 분할(B층)의 판독은 확증**이며, 비열등 마진을
판독값을 보기 전에 동결한 뒤 **T4로 1회 판정했다. 판정은 실패다**(§6.8.2). 이 절이 새
벤치마크가 아니라 **같은 벤치마크의 두 번째 판독**인 이유는 질의·정답·분할·누출 차단이 전부
같고 답을 만드는 쪽이 상수이기 때문이다.

RQ 번호도 사전등록의 것을 유지한다(RQ1은 검증 게이트였고, 그 축은 §4·§9로 강등됐다. RQ4는
...
2. **선행기술조사 태스크의 검색 유용성 실증 — 경계를 명시한 형태로.** 거절 특허와 심사관 인용을
   ...
   **다만 그 관측은 두 번째 확증 분할에서 재현되지 않았고**(§6.4a), 그래서 교차 태스크 조건의
   근거는 결함주입 실험이 진다 — 갈림을 감추지 않고 §7.3·§8.1에 그대로 적었다.
   **이 기여의 경계는 두 방향으로 확인했다.** 하나는 **같은 실험을 겹치지 않는 두 번째 확증
   분할에서 다시 한 것**이고(§6.4a — 깊은 회수의 이득과 상위 정렬의 미개선이 둘 다 재현됐다),
   다른 하나는 **한 층 아래로 내려가 본 것**이다(§6.8 — 검색 팔만 바꾸고 답을 만드는 쪽을 전부
...
범위를 먼저 보고한 뒤, 확증 분할에서 수행한 검색 성능·가설 판정·하위집단·절제 결과와
...
그리고 사전 동결한 결론 규칙을 제시한다. **8장은 그 관측들에서 다른 공학 온톨로지로 이전
가능한 설계원리 여섯을 도출하고**(DP1–DP6 · 등급과 반증 조건 포함), 가르지 못한 경쟁 설명을
"이것을 가를 실험"의 명세로 싣는다. 9장은
한계와 타당성 위협을, 10장과 11장은 데이터·코드 가용성과 결론을 제시한다.
...
이 표의 수치는 **본 논문의 모든 측정이 이뤄진 자원 세대**(상류 스냅샷 `d578bf3`)의 값이다. 이후 상류 교정으로 G0는 105,713 트리플(상류 `2839afb`)을 거쳐 115,095 트리플(상류 `39855bb`)로 다시 조립됐으나, 그 세대 위의 재측정은 별도 사전등록 아래 수행하며 본 논문의 판정에는 반영하지 않는다(§9.1). <!-- sig-history -->
...
> A-Box가 채워진 결과**로 읽어야 한다. 그리고 이 사실이 §9.1이 기술하는 H2 미검정의 진짜 이유다 —
...
모든 트리플에는 **어디서 왔는지를 적은 서명**이 붙어 있고(105,588 세대 검증), 서명이 어긋나지 않는지 보는 검사(`check_signatures.py`)가 CI(continuous integration, 매 변경마다 자동으로 도는 검사) 파이프라인에 배선돼 있다(supplementary [S1](supplementary/S1-appendices-v09.md)). 큐레이션 원천과 라이선스는 §10와 출처 매니페스트에 적는다. 특허 데이터는 KIPRIS(Korea Intellectual Property Rights Information Service, 특허정보넷) 학술정보 활용 자격으로 모으며, 공개 배포는 메타데이터만 내보내는 경로를 따른다(§3.7). <!-- sig-history -->
...
함께 지키는 조건은 둘이다. ① 평가셋을 열기 **전에** 임계를 고정한다. ② 질의 수가 너무 적은 집단은 차단 규칙에 쓰지 않는다 — 우연이 판정을 흔들기 때문이다. 거부는 "그 변경이 쓸모없다"가 아니라 **"원인을 찾고 조건을 붙여 다시 올려라"**는 뜻이다. T3가 없으면 게이트는 여러 세대를 거치며 온톨로지를 **검색에만 유리한 쪽으로 서서히 기울일 수 있다**(§9.4) — T3는 그 쏠림을 1차로 막는 장치다.
...
**두 조건은 채워졌다.** 전달 실험은 §6.8에서 각 층 1,188회 완주했고 API 실패는 0건이었다 — 계측기(모델·프롬프트·컨텍스트 개수·온도·채점 규칙)가 결과를 보기 전에 동결된 채 실제로 돌았다는 뜻이다. 비열등 마진도 **두 번째 확증 분할의 판독값을 하나도 보기 전에** 동결했다: 주 지표는 인용 정확도(전체)이고, 마진 **ε<sub>T4</sub> = 0.02**는 T1의 마진을 그대로 승계했으며 — 새 숫자를 만들면 그 출처가 결국 결과가 되기 때문이다 — 환각률 쪽 임계 **η = 0.01**은 그 절반을 걸었다. 환각은 손해가 비대칭이라 정확도 이득과 등가로 교환할 수 없기 때문이다. 마진은 코드에서 상수로 고정돼 호출자가 바꿀 수 없다.

**그리고 실제로 한 번 판정했다 — 결과는 실패다**(§6.8.2). 즉 T4는 더 이상 돌려 본 적 없는 조건이 아니다.
...
이 조건이 왜 필요한지는 사변이 아니라 관측이다 — 자원 층의 지표가 검색 층의 성능을 대표하지 못한 사례가 이 연구 안에서 두 번 나왔다(§7.5). 같은 어긋남이 검색 층과 생성 층 사이에도 있는지는 **재 보기 전에는 알 수 없었고**, 재 본 결과는 §6.8.2에 있다.
...
**한 층 아래의 판독 지표 (§6.8 · A층 탐색 → B층 확증).** 위 지표는 전부 검색 층의 것이다. 검색 층의 이득이
...
T4 판정에 쓰인 확증 지표이며 나머지 둘은 보고만 한다(§6.8.2). **어느 쪽이든 §6.2–6.4의 표와
같은 표에 넣지 않는다** — 검색 층의 지표가 아니기 때문이다. 함께 읽어야 할 맥락 하나를 표에 같이
싣는다: **질의당 평균 인용 건수**. 인용 정확도의 분모는 질의 수가 아니라 **인용한 식별자 전량**
이고 인용을 덜 하면 정확도는 저절로 오르므로, 분모를 보이지 않으면 이 넷은 읽을 수 없다(§6.8).
...
> **위 프로토콜은 다른 작업 하나에 재사용됐다 — 그러나 그것은 이 판정이 아니다.** §8.3의 실패
> 유형 분류가 이 절의 코딩 절차(가림 · 독립 코딩 · κ와 합의율 보고 · κ<0.4면 부록)를 그대로
> 가져다 쓴다. 다만 **묻는 것이 다르다** — 이 절은 *"이 후보가 관련 있는가"*를 묻고 §8.3은
...
**이 장의 증거는 층 × 역할 격자 위에 선다 (2026-08-15 추가 · 판정과 수치는 바뀌지 않는다).**
층은 §7.5가 중심 관측으로 정리한 셋이고(자원 → 검색 → 생성), 역할은 둘이다 — **후보 생성**(무엇을
검토 대상 풀에 넣는가)과 **재순위화**(풀 안에서 순서를 어떻게 바꾸는가). 절은 아래 번호 순으로
이어지지만 **각 절이 격자의 어느 칸을 채우는지는 절 순서로 드러나지 않아**, 여기에 한 번 적어 둔다.

| 층 | 후보 생성 — 풀을 만든다 | 재순위화 — 풀 안에서 다시 세운다 |
|---|---|---|
| **자원** | §6.7 — 문서집합·모델·가중치·분할을 고정하고 자원만 교체(문서당 개념 1.545 → 3.779) · **T1 실패로 승인 거부** | 자원은 두 역할 공통의 입력이므로 열을 나누지 않는다 (좌측과 같음) |
| **검색** | §6.2 단독 팔의 회수(B0·B2·B3·B5) · §6.3a 후보 상한 — **온톨로지를 후보 생성기로 세운 비교는 이 논문에 없다** | §6.2 P0★·P1 · §6.4 절제 A1–A8 · 하위집단 · §6.4a 두 번째 확증 분할 |
| **생성** | 해당 없음 — 컨텍스트는 동결된 검색 run의 상위 K이므로 생성 층은 풀을 만들지 않는다 | §6.8 전달 판독 · **T4 판정 1회 = 실패** |

**빈 칸이 이 격자의 요점이다.** 검색 층의 후보 생성 칸에는 단독 팔의 회수만 있고, **온톨로지로
후보를 만들고 텍스트로 다시 세우는 방향의 비교는 수행되지 않았다** — 제안 순위 함수가 텍스트
기준선 상위 1,000의 재정렬로 동결돼 있기 때문이다(§4.7). 이 하나의 빈 칸이 뒤에서 세 번 다른
얼굴로 나타난다 — §7.2의 재순위화 천장, §6.2f 관측 2(B5가 영어 정답에서 모든 시스템 중 가장
높은데 그 문서가 애초에 재정렬 대상 풀에 들어오지 않는다), 그리고 §8.1의 DP5(후보생성 분리).
**셋은 서로 다른 발견이 아니라 같은 구조 제약의 세 관측이다.**
...
> **이 절부터 §6.4까지는 첫 확증 분할(A층)의 값이다.** 같은 실험을 겹치지 않는 두 번째 확증
> 분할에서 다시 한 결과는 **§6.4a**에 따로 있다. 두 분할은 질의 집합도 정답 집합도 다르고
> 판정도 각각의 사전등록 아래 따로 서므로 **표를 합치거나 평균내지 않는다.** 또한 이 절의
> 값은 **교정 전 자원 스냅샷에서의 관측**이며, 이후 코퍼스에서 재산출하지 않는다 — 두 번째
> 분할을 만드는 과정에서 후보 문서 8건의 본문이 바뀌었고, 그 8건이 이 표의 값을 얼마나 옮기는지
> 모르는 채로 다시 계산하면 그것은 재측정이 아니라 새 실험이기 때문이다.

표의 모든 수치는 `python -m sdkb_paper.analysis.results_table --split {dev,test}` 의 출력이며 수기 기입이 없다(생성물: `paper/tables/ir_performance_{dev,test}.md`). 조건은 전 시스템 동일하다 — F10 후보 마스크(시점 유효·자기 및 동일 패밀리 배제) 적용 후 상위 1,000 절단, family 수준 fold-then-cut, 알려진 양성 ≥1인 질의만 매크로 평균. **주 지표는 family-level Recall@100**이며 test 분할은 최종 비교 시점에 개봉해 재선택 없이 평가했다(모든 접근은 열람 원장에 기록 — 아래).
...
**종합.** 확증 분할 정답의 29%(139/479)가 비한국어인데 최종 시스템은 그중 7건(5%)만 회수한다. §6.4의 "교차언어 이득 유의 미달"은 이제 미설명 약점이 아니라 **번역 부재(어휘)·개념링크 결손(의미)·재순위화의 한계(구조)의 합성으로 분해된 진단**이며, 세 원인은 각각 다른 처방을 가리킨다(§7.6·§9.1).

## 6.3 가설 판정표

확증 가설은 **H3·H5** 둘이다. 강등된 H1·H2·H4의 판정은 **바뀌지 않았고** 아래에 함께 싣되,
근거의 상세는 각각 §6.5·§9.1·§6.4에 있다. 잘라내기 전 전문은 supplementary
[S2](supplementary/S2-fault-injection-v09.md) §S2.1에 보존한다.

> **이 표는 첫 확증 분할(A층)의 판정이다.** 같은 두 가설을 두 번째 확증 분할에서 다시 판정한
> 결과는 **표 6.3b**에 따로 싣는다. 두 판정은 서로를 대체하지 않는다 — 각각 다른 사전등록
...
| **H2** (승인 안전성) *강등 — 미검정* | 동일 파이프라인에 O와 O′를 넣어 \(\Delta R@100\)의 LB95 > \(-\epsilon\) 이고 하위집단 최대 하락 < \(\delta\) | **자격 있는 델타가 존재하지 않는다** — G0·G1·G2의 T-Box가 완전히 동일해(클래스 103 · 술어 97/81 · 델타 0) 버전 델타를 만들 수 없었다 | **미검정**(§9.1). 아래 보고한 게이트 실행은 **서로 다른 두 시스템**의 비교이므로 H2의 증거가 아니다 |
...
**표 6.3b. 같은 두 가설의 두 번째 확증 분할(B층) 판정 — 근거는 §6.4a.** 판정식·마진·주 지표는
...
| **H5** (특이성 — 음성 대조군) | A8(전문가 매칭 계층 제거)의 ΔR@100 = **0.0000** (*p*=1.000) | **판정 불가 — 재현되지 않았다** | A층에서는 +0.0316(유의 악화)이었다. **두 표본에서 갈렸고**, B층의 0은 "영향 없음"과 "뺄 개념이 없음"을 가르지 못한다(§6.4a) |
...
| 거절 유형별 신호 | 신규성은 single-reference, 진보성은 set coverage 포괄의 설명력 | **검정 불가** — 자원에 신규성 단독 거절 0건(1,000건 중 신규성 14건 전부 진보성과 병존, 확증 분할 n=3). §6.4·§9.1 |
...
| 한 층 아래로의 전달 (RQ5) | 검색 팔만 바꾸고 답을 만드는 쪽을 고정했을 때의 인용 정확도·환각률 | **A층은 탐색적 · B층은 확증**(사전 동결 마진으로 T4 판정 1회 = **실패**). 두 층 모두 온톨로지 포함 팔의 인용 정확도 점추정이 높으나 비열등을 보이지 못했고, **환각률과 근거 문장 일치는 두 표본에서 방향이 갈렸다.** §6.8 |
...
> **이 절은 §6.2–6.4를 대체하지 않는다.** 두 분할은 서로 다른 질의 집합이고, 판정도 각각의
> 사전등록 아래 따로 선다. 앞 절의 값은 **첫 확증 분할(A층 · 질의 198)**의 관측이고, 이 절의
> 값은 **두 번째 확증 분할(B층 · 질의 198 · 정답 503건)**의 관측이다. 두 표를 합치거나 평균내지
> 않는다.
...
**표 6.4a-1. B층 검색 성능 — 198질의 · qrel 503건.** 생성물 `paper/tables/ir_performance_test_b.md`.

| 시스템 | R@100 | Δ vs B3 | 95% CI | *p* | 승/패/동 | Δ nDCG@20 | *p* |
|---|---:|---:|---|---:|---|---:|---:|
| Text Hybrid (B3 · 기준선) | 0.4102 | — | — | — | — | — | — |
| BM25-Claim (B0) | 0.3254 | −0.0848 | [−0.1229, −0.0489] | .000 | 9/40/149 | −0.0240 | .028 |
| Dense (B2) | 0.3646 | −0.0456 | [−0.0831, −0.0090] | .016 | 17/32/149 | −0.0513 | .000 |
| CPC/IPC only (B4) | 0.3012 | −0.1090 | [−0.1745, −0.0458] | .001 | 35/73/90 | −0.0763 | .000 |
| Ontology-only (B5) | 0.1470 | −0.2633 | [−0.3249, −0.2012] | .000 | 20/104/74 | −0.1756 | .000 |
| Text+Ontology (P0★ · 사전지정 주) | 0.4344 | +0.0242 | [−0.0084, +0.0574] | **.147** | 25/18/155 | −0.0218 | .210 |
| **+ClaimFeature (P1)** | **0.4445** | **+0.0343** | **[+0.0094, +0.0615]** | **.004** | 19/9/170 | −0.0136 | .390 |
...
**그런데 A층의 구조가 그대로 재현됐다 — 그것이 이 분할의 실제 소득이다.** 두 문장이 독립
표본 둘에서 같이 나왔다. ① **이득은 깊은 회수에만 있고 상위 정렬에는 없다** — R@100은 오르고
nDCG@20은 오르지 않는다(A층에서도 그랬다). ② **사전 지정한 주 구성 P0★은 두 번 다 유의 미달**
(A *p*=.181 → B *p*=.147)이고 **부차 구성 P1만 두 번 다 유의**하다(A +0.0534 → B +0.0343).
효과의 크기는 B층에서 약 3분의 2로 작아졌는데, 온톨로지 신호가 얇은 표본이라는 위 성질과
방향이 같다.

**표 6.4a-2. B층 절제 A1–A8** (P1 기저 · 제거손실 Δ = full − ablated · Holm m=8).
...
않는다 — **두 표본에서 결과가 갈렸다고 쓴다.** §8.1 DP3의 근거 열에도 같은 문장을 적었다.

**표 6.4a-3. B층 하위집단**(탐색적 · 안전성 판정 없음). 생성물 `paper/tables/ir_subgroup_test_b.md`.
...
(§6.4). 탐색적이므로 확증으로 쓰지 않되, **이 결과가 §8.3 실패 유형 분류의 1순위 입력**이다.
...
④ 민감도 격자에서 부호 유지 — 미실행 ⑤ 누출 감사 0 · 봉인 접근 통제 — 누출 **0**, 그러나
**개봉은 아래와 같이 3회 시도됐다.**

> **봉인 개봉 원장 — 줄여 적지 않는다.** 첫 시도와 두 번째 시도는 각각 코드 결함과 집계 표기
> 오류로 중단됐고 **판정도 출력도 0건**이었으며, 세 번째 시도의 세 단계(성능표·하위집단·절제)가
> 각각 봉인을 열어 이 절의 값을 냈다. **결과를 보고 조건을 고친 적은 없다**는 점에서 "1회 개봉"의
> 취지는 지켰지만, 세 명령이 각각 봉인을 여는 구조 자체가 결함이며 — 개봉 횟수를 셀 수 없게
> 만든다 — 상류가 아니라 이 저장소의 수정 대상으로 등재했다.
>
> **실측 교정 (2026-08-22 · PLAN-068 A-5b).** 앞 문단은 원래 *"열람 원장은 5행이고 그것이
> 기록이다"* 라고 적었다. 그 값은 이 A층 판독 시점의 것이고, 현 원장
> (`data/processed/ir/seal_access.jsonl`) 실물은 **25행**이다(2026-08-08~18 · 확증 계열 11 ·
> 탐색적 명시 14). **더 중요한 사실은 수가 아니라 층이다 — 25행은 전량 판독 B 봉인이며 위에
> 서술한 A층 세 시도는 원장에 한 줄도 남아 있지 않다.** `analysis/metrics.py:52` 가 `test_b`
> 에만 봉인 경로를 주고 `test` 는 `QREL_EXAMINER` 를 직접 읽어 `open_sealed()` 를 지나지 않기
> 때문이다. 곧 A층 개봉 이력의 근거는 원장이 아니라 PLAN-031 기록이다. **판정과 수치는 하나도
> 바뀌지 않았고**, 바뀐 것은 감사 장치의 적용 범위에 대한 진술이다. 배선 교정은 별도 작업이다.
...
> 사실 하나이며, "온톨로지가 진화해도 안전하다"는 명제는 §9.1이 밝히는 대로 이 논문에서
> 검정되지 않았다.

두 가지를 함께 읽어야 한다. 첫째, 주 태스크 CQ가 5개뿐이라 한 질의의 응답 여부가 곧 0.2의
진폭이 된다 — 그 둔감함이 청구항 수준 분해를 선결 과제로 남기는 이유다(§9.4). 둘째, 판정 규칙이
...
그래프에 무반응인 상수항이므로 **게이트가 아니라 측정으로 운용한다**(§9.4).
...
### 6.8.1 A층 — 탐색적 판독 (계측기 동결이 목적)

**기대 방향을 미리 적어 두었다.** 검색 층에서 P1의 이득은 깊은 회수에 한정되고 얕은 깊이에는
없다(§6.2·§6.3a). 상위 10건만 쓰는 이 실험에서 **P1이 이길 것으로 기대하지 않았다.**
...
> 소급 수정하지 않는다(CLAUDE.md §1-3). 분모가 움직인다는 사실 자체는 §5.1과 §6.8.1 ①이 이미
...
**표 6.8. T4 판정 (B층 · 198질의 페어드 · 부트스트랩 10,000회).** 생성물
...
않는 보조 지표이므로 확증으로 읽지 않되, **§8.3 실패 유형 분류의 입력으로 넘긴다.**
...
**설계원리에 주는 함의(§8.1 DP6).** 이번 판정은 DP6 — *검색 지표의 개선을 생성 답변 품질로
자동 일반화하지 않는다* — 을 **지지하는 방향**이다. 검색 층에서 Recall@100이 유의하게 개선된
팔이 생성 층에서는 비열등 판정을 통과하지 못했다. 자동 일반화가 성립하지 않는 이음매가
자원→검색(§6.7)에 이어 검색→생성에서도 관측된 셈이다. **그럼에도 DP6을 확립으로 올리지 않는다** —
...
> 확정적이지만 **인과는 미구분**이고(아래 (가)·(나)), 한 확증 분할에서 나온 결과이며, 이 릴리스의
...
않는 질의 집합에서 다시 하자 A8의 제거 손실은 **정확히 0.0000**이었다(§6.4a). 두 값을 평균내
...
보유율을 분해해 비교하는 것이고, 명세는 §8.2에 적었다.
...
관측이다(§6.5). 설계원리 표(§8.1)의 DP3도 같은 순서로 근거를 다시 적었다 — 결함주입 결과가
...
**① 자원 층 → 검색 층.** 상류 교정으로 문서당 개념이 1.545개에서 3.779개로 **2.4배**가 됐고 개념 어휘도 141개에서 199개로 늘었다. 자원 품질 지표는 모두 좋아졌다. 그런데 같은 파이프라인에 이 자원을 갈아 끼우자 주 시스템의 회수는 **떨어졌다**(family R@100 −0.0293, 95% 신뢰구간 [−0.0542, −0.0053]). 형식 검증 L0–L3를 전부 통과한 변경을 **성능 조건 T1이 거부했다**(승인 거부). 전문과 판정은 §9.1.2에 있다.
...
**셋을 관통하는 것은 하나다 — 어떤 층의 지표도 다음 층의 성능을 보장하지 않는다.** 자원이 풍부해지면 문서당 개념이 늘고, 특이도 가중이 없는 겹침 점수에서는 고빈도 일반 개념이 변별력 없는 동점 덩어리를 만든다(§9.1.2). 이것은 자원의 실패도 방법의 실패도 아니라 **두 층이 서로 다른 것을 재고 있다**는 사실의 귀결이다.
...
**그 세 번째 층은 두 번 쟀고, 두 판독의 방향이 서로 달랐다(§6.8).** 답을 만드는 쪽을 전부 고정하고 검색 팔만 바꾼 판독에서, 온톨로지를 포함한 팔은 두 층 모두 인용 정확도 점추정에서 앞섰다. 그런데 **두 번째 판독은 사전에 마진을 동결하고 실제로 판정했고, 그 판정은 실패했다** — 신뢰구간 하한이 비열등 마진을 0.0005 넘겼다. 즉 **점추정으로는 어긋남이 없었는데 판정으로는 비열등조차 보이지 못했다.** 게다가 보조 지표 둘(환각률·근거 문장 일치)은 두 표본에서 **부호가 반대**로 나왔다. 이 결과를 위 세 관측 옆에 네 번째 어긋남으로 세우지도, 반례로 세우지도 않는 이유는 하나다 — **실패의 원인이 전달의 부재인지 검정력의 부족인지 이 실험은 가르지 못한다.** 그러나 이 절의 주장은 그대로다. 이 절이 주장하는 것은 "다음 층은 항상 나빠진다"가 아니라 **"다음 층의 결과는 예측할 수 없으므로 재야 한다"**이며, 점추정과 판정이 서로 다른 답을 내놓았다는 사실이 그 말의 가장 직접적인 형태다.
...
이 절의 세 관측 중 확증 지위를 갖는 것은 **하나도 없다.** ①은 개봉된 확증 분할 위에서 사전등록된 승인 규칙을 적용한 것이고, ②는 탐색적 기술통계이며, ③은 일화다. 그러므로 이 절이 주장하는 것은 "온톨로지 보강은 무익하다"가 아니다 — 실제로 온톨로지 단독 검색팔은 같은 교정으로 27% 개선됐다(§9.1.2). 주장하는 것은 **결과가 예측 불가능하므로 층마다 재야 한다**는 것이다.
...
**한 층 아래에 대한 결론 규칙은 위 두 행이 전부다.** §6.8의 두 번째 판독은 확증이지만
...
앞 장까지는 **이 산출물에서 무엇이 관측됐는가**였다. 이 장이 묻는 것은 하나다 —
**그중 무엇이 다른 공학 온톨로지로 가져갈 수 있는가?**
...
**다섯을 모두 채우면 "확립", 하나라도 미달이면 "제안"으로 강등한다.** 아래 여섯 중 다섯이
...
|---|---|---|---|---|---|
| **DP1** | **층별 검증** — 온톨로지 내부 품질과 하류 태스크 성능은 **별도 층에서** 평가한다 | §6.7 — 문서당 개념 1.545 → 3.779(2.4배)인데 주 시스템 Recall@100 은 −0.0293 | 확인이 먼저 | 2026-08-01 | **확립** |
| **DP2** | **한 층 아래 승인** — 자원 변경은 자원 지표가 아니라 **다음 사용 층의 비회귀 결과**로 승인한다 | §6.7 — 형식 검증 L0–L3 를 **전부 통과한** 델타를 성능 조건 T1 이 거부(Accept = 0) | **§4.9 설계 시점** | 2026-08-01 | **확립** |
| **DP3** | **교차 태스크 감시** — 공유 T-Box 에서는 **주 태스크 성능만으로** 변경을 승인하지 않는다 | §6.5 — 교차 결함을 T3 만 단독 검출(12/45 · 단측 *p*=.0001) · §6.4 — 음성 대조군(A8) 제거가 검색을 −0.0316 악화, 다만 **§6.4a 두 번째 분할에서는 0.0000 으로 갈렸다** | **§4.9 설계 시점**(T3) | 2026-07 | **확립** — 근거는 결함주입 T3 단독 검출이 진다. A8 근거는 **두 표본에서 갈렸음을 그대로 적는다**(경쟁 설명 §8.2) |
| **DP4** | **통제된 자원 교체** — 문서집합·모델·가중치를 고정하고 **자원만 교체**해야 온톨로지의 효과가 판별된다 | §9.1.1 — T-Box 가 한 번도 바뀌지 않아 H2 를 **원리적으로 잴 수 없었다** · §9.1.2 — 두 팔이 처음 성립한 뒤에야 판정이 나왔다 | 확인이 먼저 | 2026-08-01 | **확립** |
| **DP5** | **후보생성 분리** — 재순위화는 후보 풀 **밖**의 문헌을 복구할 수 없으므로, 후보생성과 **구분해** 평가한다 | §6.3a — 상한이 없을 때 세 시스템의 미도달 질의 비율이 **완전히 동일**(26.8 % · 55.1 %) | 확인이 먼저 | 2026-08-02 | **확립** |
| **DP6** | **전달 검증** — 검색 지표의 개선을 검토 효율이나 생성 답변 품질로 **자동 일반화하지 않는다** | §6.3a — Recall@100 이 +0.0534 인데 Candidate Reduction 중앙값 **0.0 %** · §6.8 — 검색 층에서 유의하게 개선된 팔이 **생성 층 비열등 판정을 통과하지 못했다**(T4 실패 · 하한 −0.0205 대 마진 −0.02) | §4.9.1(T4 설계) | 2026-08-09 (확증 판정 1회) | **제안** |
...
§6.7 의 관측(자원을 2.4배 개선했는데 검색이 나빠졌다)에는 경쟁하는 설명이 다섯 있고,
...
| 경쟁 설명 | 예측 | 이것을 가를 실험 | 현 상태 |
|---|---|---|---|
| **음성 대조군(A8)의 두 분할 불일치** — ⓐ A층의 유의 악화가 우연이었다 ⓑ B층 질의에 전문가 계층 축의 개념이 없어 뺄 것이 없었다 | ⓑ가 맞다면 B층 질의의 해당 축 개념 보유율이 0에 가깝다 | 두 분할의 질의별 **축별 개념 보유율**을 분해해 비교 | **미수행** — §6.4a는 "재현 실패"와 "잴 대상 부재"를 가르지 못한다 |
| **전달 판정 실패의 원인(T4)** — ⓐ 검색 이득이 생성 층으로 전달되지 않는다 ⓑ 표본이 작아 비열등을 보이지 못했다 | ⓑ가 맞다면 표본을 늘릴 때 하한이 마진 안으로 들어온다 | 질의 수를 늘려 같은 계측기로 재판정(새 사전등록) | **미수행** — 하한이 마진을 0.0005 넘긴 아슬아슬한 실패라 두 설명이 모두 살아 있다(§6.8) |
...
**먼저 가른다 — 이것은 §5.5의 전문가 판정이 아니다.** §5.5가 설계해 둔 것은 *"정답 목록에 없는
상위 후보가 진짜 오탐인가"*를 묻는 **관련성 판정**이며 여전히 **미수행**이다. 이 절의 작업은
관련성을 판정하지 않는다 — 이미 정답으로 확정된 문헌이 왜 밀렸는지를 **관측된 특징으로 분류**할
뿐이다. 코딩 프로토콜(가림·독립 코딩·κ 보고·κ<0.4면 부록)만 §5.5에서 그대로 가져다 쓴다.
...
**표 8.3. 사람 코딩 40쌍의 유형 분포**(두 분할 합계 · 전문과 분할별 분해는 부록).
...
못했다.** 채운 것은 DRQ3·DRQ4의 정성 축이고, §8.1의 DP 등급은 하나도 바뀌지 않는다.
...
**위 §9.1.1은 본 논문 자원(스냅샷 `d578bf3`)에 대한 판정으로 그대로 유지된다.** 아래는 그 처방을
...
- **자유 텍스트 질의를 받을 수 없다.** 본 벤치마크가 성립하는 것은 질의가 전량 **이미 온톨로지에
  ...
  붙일 진입로가 없다. 이는 성능의 한계가 아니라 **적용 범위의 한계**이며, §9.3에 외적 타당도
  항목으로 따로 적는다.
- **개념 어휘를 늘려도 신규 문서에는 적용되지 않는다.** 자원 측 보강의 효과는 링킹 단계가 생긴
  뒤에야 새 문서로 전파된다.

**이 제약은 그 뒤 해소됐고, 해소한 상태에서 다시 잰 결과가 §6.7에 있다(2026-08-15 상호참조).**
위 세 항목은 2026-08-02 시점의 기록이며 **소급 수정하지 않는다.** 다만 상류가 개념 사전을 발행하고
하류에 적용기가 붙은 뒤 링킹 단계는 실제로 생겼고, 문서집합·모델·가중치·분할을 고정한 채 자원만
갈아 끼워 다시 쟀다 — 문서당 개념은 1.545에서 3.779로 늘었고 온톨로지 단독 팔(B5)의 회수는
0.1800에서 0.2282로 올랐으나, **교체 대상 구성(P1)의 회수는 −0.0293만큼 떨어져 T1이 실패했다**
(§6.7). 따라서 이 절을 *"링킹 단계를 만들면 검색이 개선된다"*의 근거로 읽지 않는다 — 만들어
봤고, 그 결과가 §6.7이다. 링킹 단계의 부재는 **적용 범위의 한계**이지 관측된 성능 부족의 확인된
원인이 아니며, 원인이 자원인지 하류 점수식인지는 여전히 **미구분**이다(§8.2).
...
파생 통계로 재현 경로를 제공한다(§10).

**질의 모집단의 제약 — 외적 타당도(2026-08-02 추가).** 본 평가의 질의는 전량 **이미 온톨로지에
등재된 특허**이며, 그래서 질의 측 개념 링크를 그래프에서 조회할 수 있다(§9.2). 실무의 선행기술
...
§6.8은 답을 만드는 층까지 두 번 내려가 본 판독이며, 한계가 여럿이다. 몇은 그 절이 이미 본문에
...
정확한 진술이고, **전달의 부재인지 검정력의 부족인지는 미구분**이다(§8.2). 가르려면 질의 수를
...
- **전달 판독의 계측기 전량(§6.8)** — 답을 만드는 쪽에서 고정한 값 전부(모델 ID
  ...
  **출력 절단율 0.400**을 보고 1024에서 4096으로 올렸고(길이만 바꿨다), 응답 파서는 **파싱 실패율
  ...
  이며 판독값을 보고 한 변경이 아니다 — 후자는 수리 전후 값을 §6.8에 병기했다.
...
- **실패 유형 분류의 코딩 규약(§8.3)** — 유형 정의 F1–F7 · 코딩 시트가 보여주는 것과 가리는 것 ·
...
다만 이 셋의 부재가 재현을 막지 않는다 — §10.2 의 식별자와 재구축 절차로 동일한 코퍼스를 다시
만들 수 있으며, 저장소 버전과 논문 수치의 일치는 CI 에서 검사한다. 본 논문이 쓴 좌표는 상류 스냅샷 `d578bf3` 과
그 `PROVENANCE.json` 의 파일별 sha256 이며, 데이터 릴리스 DOI 는 공개 릴리스 시점에 부여한다.
...
봉인 분할에서 수행한 사전등록된 확증 평가는 두 가지를 남긴다. **첫째, 검색 유용성은 주 지표에 한정해 지지된다(H3).** 온톨로지·한정요소 재순위화는 최강 텍스트 하이브리드 대비 family-level Recall@100을 +0.0534(*p* = .008) 개선했으나, 사전 지정한 주 순위 함수는 유의 수준에 미달했고(*p* = .181) nDCG@20은 개선되지 않았으며, 이득은 예상과 반대로 고어휘중첩 질의에 몰렸다. 절제는 이득의 본체가 계층 심화가 아니라 개념 겹침 하나임을 가리킨다. **둘째, 음성 대조군이 무너졌다(H5).** 검색과 무관하도록 설계한 전문가 매칭 전용 계층을 제거하자 검색이 유의하게 악화되어(+0.0316, *p* = .002), 특이성 대신 **경험적 교차 태스크 의존성**이 관측되었다.

**이 둘을 겹치지 않는 두 번째 확증 분할에서 다시 판정했고, 결과는 갈렸다(§6.4a).** H3 쪽은 구조가 재현됐다 — 깊은 회수의 이득(+0.0343, *p* = .004)과 상위 정렬의 미개선, 주 순위 함수의 유의 미달(*p* = .147)이 그대로다. 다만 사전등록이 두 지표의 동시 개선을 요구하므로 그 분할의 판정은 **미지지**다. H5 쪽은 재현되지 않았다 — 같은 절제의 값이 **0.0000**이다. 두 판정 중 어느 것도 소급해 취소하지 않으며, 갈렸다는 사실 자체를 결과로 싣는다. 그 결과 교차 태스크 조건의 근거는 음성 대조군이 아니라 **결함주입 실험이 진다**(홀드아웃 45건 중 T3 단독 검출 12건 · 단측 *p* = .0001).
...
> 강등된 H1·H2·H4 는 판정을 **그대로** 적되 싣는 자리만 옮겼다(§4·§6.4·§9).
...
| 승인 델타의 비열등·하위집단 안전 (H2) | 동일 파이프라인 O 대 O′ | **미검정** — 자격 있는 T-Box 델타가 존재한 적 없다 | §9 |
| 미인용 상위 후보 일부가 관련 | 전문가 표본 판정 | 미실험 | §9 |

**두 번째 확증 분할과 한 층 아래의 판정은 별도 사전등록 아래에서 나왔다 — 위 행을 덮어쓰지 않고 따로 적는다.**

| 주장 | 증거 유형 | 판정 | 위치 |
|---|---|---|---|
| **하이브리드가 텍스트 기준선 개선** (H3 · 두 번째 확증 분할) | 시스템 비교 | **미지지** — 깊은 회수는 개선되나 nDCG 조항 미충족 · 주 구성 비유의 | §6.4a·§6.3 표 6.3b |
| **음성 대조군 무영향** (H5 · 두 번째 확증 분할) | A8 절제 | **재현되지 않음** — 두 표본에서 갈렸고 "영향 없음"과 "잴 대상 부재"를 가르지 못함 | §6.4a·§7.3 |
| **검색 이득의 생성 층 전달** (T4 · 확증) | 검색팔만 바꾼 생성 판독 · 사전 동결 마진 | **실패 — 전달을 확증하지 못함**. 점추정은 제안 팔 우세이나 신뢰구간 하한이 마진을 넘김 · 원인 미구분 | §6.8.2·§4.9.1 |
| 온톨로지 팔이 악화시킨 질의의 **실패 유형 분류** (C4) | 가림 코딩 40쌍 + LLM 전수 코딩 | **산출 · 부록 이관** — 사전 동결한 κ 문턱 미달 | §8.3 |
