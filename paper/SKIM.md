# 1. 서론
  [그림 1] 자원 지표는 개선되었고 형식 검증 네 층을 전부 통과하였으나, 사전 지정 비열등 기준을 충족하지 못하여 그 변경은 거부되었다.
# 2. 배경과 연구 공백
...
## 3.1 공유 공정 노드를 지나는 세 태스크 경로
  [그림 3] 세 태스크 뷰는 별개의 온톨로지가 아니라 같은 공정 노드를 서로 다른 술어로 지나는 질의 경로이며, 그 자원은 계속 갱신된다.
...
## 3.5 형식 층과 태스크 조건의 결합 승인식
  [그림 4] 형식 검증 네 층을 통과한 변경도 태스크 조건에서 멈추며, 각 항은 미충족 시 뒤를 실행하지 않는다.
