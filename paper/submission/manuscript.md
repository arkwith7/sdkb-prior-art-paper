...
확인되지 않으며, 이 축은 확증 가설이 아니므로 탐색적 진단으로만 보고한다(§5.4.3).
...
있으며, 생성 단계의 지표는 검색 층의 지표가 아니므로 §5.4의 표에 포함하지 않는다.
...
별도로 등록되었으므로 아래 표는 예측과 판정을 분할별로 구분한다. 데이터·온톨로지·shape·CQ·인덱스·
모델 버전은 해시로 고정하였고 시드와 lockfile, 분할 식별자 목록을 공개한다. 점검 명칭과 등록
문서의 대응은 [S6](../supplementary/S6-preregistration-crosswalk.md)에, 사전등록·봉인·개봉의
이력은 S5에 있다.

**표 4. 사전등록된 평가 점검과 그 판정 — 근거와 전문은 5장·supplementary S5.**
...
태스크의 평가 자산을 갖추었으나 자격을 갖춘 자원 변경이 한 건뿐이었다(§3.2 · §5.1). 제2 자원은
...
본 장이 보고하는 발견은 다섯이다. 형식 검증 네 층을 전부 통과한 실제 자원 변경을 성능 조건 T1이
거부하였다(§5.1). 형식 검증과 주 태스크 성능 검사가 모두 놓친 교차 태스크 결함은 조건 T3이
단독으로 검출하였다(§5.2). 온톨로지 재순위화의 이득은 깊은 회수에 존재하며 겹치지 않는 두 확증
분할에서 반복 관측되었고(§5.4.1), 그 이득의 경계도 정량화되었다. 상위 정렬은
개선되지 않았고 검토 건수로 환산한 이익은 소멸한다(§5.4.1 · §5.4.3). 형식 층과 교차 태스크 층은
제2 공학 온톨로지에서 코드 변경 없이 실행되었으나, 동결한 결함 명세는 그 자원의 모델링 관습에
맞추어 재정의되어야 하였다(§5.5).

제시 순서는 에피소드 번호가 아니라 발견의 무게를 따르며, 각 절의 첫 문단은 그 절의 결론과 확증
지위를 함께 진술한다. 에피소드별 지위는 표 2에, 사전등록된 점검의 판정은 §4.5의 표 4에 있다.
...
본 절은 별도 사전등록 아래의 판정이고 자원 스냅샷도 교정 후 세대이므로, §5.4의 확증 판정은 이
...
12/45 는 τ=0.10에서 4/45로 감소하고 τ=0.00에서 17/45로 증가한다(표 9). 사전 지정 임계에서도
...
본 에피소드의 결론은 이득이 관측된 범위이다. 온톨로지 재순위화는 사전에 기대하였던 세 자리, 곧
어휘 불일치 질의와 상위 정렬, 검토 효율에서 개선을 보이지 않았다. 개선이 관측된 자리는 깊은 회수
하나이며 그 이득도 운용 단위로 환산하면 소멸한다. 본 에피소드는 봉인 분할에 대하여 사전등록된
확증 평가를 두 차례 수행하였다. 두 분할은 겹치지 않으며 각각의 사전등록 아래에서 개별적으로
...
기준선의 값은 확증 판정에 들어가지 않는다(단독 구성 넷의 행을 포함한 전체 결과는 S5). 분류 단독과
개념 단독의 R@100은 B3보다 약 0.25 낮다. 곧 이 결과는 온톨로지를 텍스트 순위와 결합하는 방식의
효과를 보여준다. 효과 크기는 패널 B가 패널 A의 약 3분의 2 수준이다.
...
보정 후 유일하게 유의하였으며(표 7), 판정은 기각, 곧 교차 태스크 의존성의 관측이다. 두 번째
...
기여는 별도의 절제 행이 아니라 표 6의 구성 간 비교가 진술한다.
...
실제 릴리스 계보에서는 인접 릴리스 다섯 쌍을 이행 조건 둘로 판정하였다(표 8). 부분 승인은 두
...
**표 8. 제2 공학 온톨로지의 릴리스 계보 판정 — 형식 층과 교차 태스크 층에 한정한다.** d0–d5는
...
거부하였다(−0.0293 · 95% CI [−0.0542, −0.0053] · §5.1 · 사전등록된 승인 규칙의 적용). ② 검색
층에서 운용 단위로는, 주 지표의 이득이 검토 건수 단위로 환산하면 소멸한다(감소율 중앙값 0.0% ·
승·무·패 62/16/64 · §5.4.3). ③ 검색 층에서
...
매칭 함수가 필요하다. 이 상한은 교차언어 축에서 가장 크게 관측되었다(§5.4.3).
...
않았고(§5.4.2), 두 표본이 갈린 원인도 구분하지 못하였다. 따라서 이 교훈의 주된 근거는 결함주입
...
취한 규범적 선택이다(§3.5). 표 9는 각 판정이 전환점에서 얼마나 떨어져 있는가를 보고한다. T1의
거부는 여유가 크고, T4의 실패는 가장자리에 있으며, T3의 검출은 분포 검사 임계에 민감하여 사전
동결 격자의 상단에서 지지를 잃는다.

**표 9. 결정 안정성 — 동결한 임계에서 판정이 전환되는 지점.**
...
본 절은 본 연구의 검증 강도와 일반화를 제약하는 결손을 표 10에 모아 제시하고, 표로 옮기기 어려운
넷만 산문으로 다룬다.
...
**결손의 명세.** 아래 표는 검증 강도와 일반화를 제약하는 결손 아홉과 해소 명세이다. 그 가운데
주장의 범위를 가장 크게 제약하는 셋은 승인 안전성의 미검정(②)과 전문가 관련성 판정의
미수행(⑧), 질의 언어와 정답 언어의 불일치(⑥)이다.

**표 10. 검증 강도와 일반화 가능성의 결손 아홉 — 해소 방법은 전부 새 사전등록의 대상이다.**
...
| ③ | 교차 태스크 검출의 임계 민감성 | τ=0.05에서 T3 단독 검출 12/45이나 τ=0.10에서는 4/45, τ=0.00에서는 17/45 (§5.2) | τ 격자 전반의 검출률·위양성률 곡선을 사전등록하고 보고 |
...
| ⑦ | 강한 다국어 기준선의 부재 | **부분 해소** — 다국어 융합 기준선을 추가하였고 기준선 강도는 유의하게 변하지 않았다 (§4.3 · 표 6). 두 인코더가 같은 백본 계열이므로 **계열 다양성은 미달성** | 계열이 다른 인코더를 포함한 재평가 |
| ⑧ | 전문가 관련성 판정 미수행 | 프로토콜은 동결하였으나 판정은 수행하지 않았다 (§4.4). 취약도는 판정 전환 최소 건수로 계량하고 외생 라벨 병합으로 **부분 축소**하였다 (§5.4.1) | 상위 미인용 후보 **표적 표본**의 2인 가림 독립 판정과 κ 보고 |
...
명제이다. 본 연구의 판정은 이 방향을 지지하나 실패의 원인을 구분하지 못하였다(실패의 폭은 표 9).
