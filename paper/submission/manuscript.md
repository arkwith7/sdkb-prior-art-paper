...
텍스트 질의를 개념에 연결하는 단계가 파이프라인에 존재하지 않기 때문이며, 본 장이 측정하는 것은
그 조건 아래의 성능이다(§6.5). 주 분석의 질의 단위는 거절 특허 1건이며, 질의 본문은 독립항
...
<!-- style-ok: 시스템 정의 열거 -->
본문이 사용하는 비교 구성은 넷이다. **B3** Text Hybrid(BM25와 Dense를 순위 융합, reciprocal rank
fusion · **가장 강한 텍스트 기준선**) · **B5** Ontology-only(개념 경로 단독) · **P0** Text+Ontology
(B3 + 개념 겹침·경로 · **사전 지정 주 구성**) · **P1** +ClaimFeature(P0 + 한정요소 포괄 · **부차
구성**)이다. B3가 가장 강한 텍스트 기준선이라는 진술의 분해 근거는 어휘 단독(B0)·밀집 단독(B2)·
분류 단독(B4)의 세 구성이며, 세 구성의 수치는
...
후보 집합을 확대하지 않으며, 이 설계 선택이 §6.2가 진단하는 재순위화 상한의 원인이다.
...
**표 4. 사전등록된 평가 점검과 그 판정 — 근거와 전문은 5장·supplementary [S5](../supplementary/S5-submission-full-v2.md).**
...
**표 5. 자원만 교체하였을 때의 검색 성능 (test 198질의 · family Recall@100).**
...
**표 6. 두 확증 분할의 검색 성능 (2-panel) — 기준선은 각 패널의 Text Hybrid(B3)이고, Δ·CI·*p*·승/패/동은 질의 단위 paired bootstrap 10,000회다.**
...
판정은 §4.5의 표 4에 분할별로 제시되어 있다. 검색 유용성 점검의 사전등록은 두 조건을 요구하였다.
...
**표 7. 첫 확증 분할의 하위집단과 절제** (test 198질의 · family R@100 · 질의 단위 paired bootstrap
...
온톨로지 전체의 기여는 별도의 절제 행이 아니라 표 6의 구성 간 비교가 진술한다.
...
![그림 7. 검토 깊이별 회수 곡선 — 검토 깊이 K에 따른 family Recall@K 곡선(로그 축).](../figures/ir_effort_curve.svg)

**그림 7.** 검토 깊이별 회수 곡선. 목표 회수에 도달하지 못한 질의는 제외하지 않고 상한+1건으로
...
**표 9. 검증 강도와 일반화 가능성의 결손 여덟 — 해소 방법은 전부 새 사전등록의 대상이다.**
