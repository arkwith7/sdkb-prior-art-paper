...
본 연구는 설계과학연구(design science research, DSR) 접근으로 두 산출물을 설계한다. 첫째는 반도체 도메인 온톨로지 데이터셋 **SDKB**(Semiconductor Domain Knowledge Base)이다. SDKB는 전문가 매칭·선행기술조사·기술예측 세 태스크를 하나의 공유 스키마(T-Box) 위에 태스크별 뷰로 표현한다. 둘째는 형식 검증 네 층(L0–L3) 위에 태스크 조건 셋을 결합한 **변경 승인 게이트**이다. 세 조건은 검색 성능의 비열등성(T1), 하위 집단의 국소 회귀 방지(T2), 다른 태스크의 역량질문 통과율 비회귀(T3)이다.
...
낸다는 의미가 아니다. 공유 어휘의 골격(T-Box)과 식별자를 유지한 채, 태스크마다 필요한
...
아래로 내려갈 수도 있다. 본 논문은 이 현상을 **과제 의미 회귀(task-semantic regression)** 라
...
즉 어휘가 얼마나 증가했는지, 문서 한 건에 개념이 몇 개 결합되는지, 누락된 링크를 얼마나
보완했는지를 측정한다. 여기서 **자원(resource)** 은 응용에 투입되기 이전 상태의 데이터셋 전체를
가리킨다(T-Box·A-Box·문서–개념 링크). 그러나 평가에는 세 개의 층이 존재한다. 어휘·링크·해상도를
...
Brank et al., 2005). 다만 이 전통에서 과제 성능은 여러 온톨로지를 비교하여 선택하는
기준(selection criterion)으로 사용되었다. 본 연구가 이동시키는 것은 그 사용 위치이다. 본 연구는
동일한 과제 성능을 하나의 온톨로지가 성장할 때 그 변경의 수용 여부를 결정하는 승인식의 항으로
...
scope)** 는 T-Box·SHACL·CQ가 세 태스크의 질의를 표현하고 실행할 수 있는가를 가리킨다. **과제
...
이상의 검토에서 도출되는 연구 공백은 다음과 같다. 여러 태스크를 함께 지탱하는 도메인 온톨로지를 대상으로 한 설계에
관한 것이다. 단일 태스크만 관찰하는 게이트가 유발하는 과적합을 **교차 태스크 비회귀(cross-task
...
L0–L3 위에 과제 조건 T1·T2·T3를 결합한 병합 전 승인식이며, T4는 승인식 외부의 설계이다
(§4.4–4.5). **A3 · 다층 평가 벤치마크**는 누출을 차단한 심사관 정박 검색 평가에서 그 하위의 생성
층 판독까지를 연결하는 계측 장치이다(§5).

설계와 평가는 교대로 순환한다. 자원을 수정하면 게이트가 그 변경을 심사하고, 게이트가 놓치는
부분은 벤치마크가 드러내며, 벤치마크가 드러낸 결손은 다시 자원 수정으로 환류된다. 이 순환에서
평가는 한 번의 최종 검정이 아니라 목적이 서로 다른 복수의 에피소드로 구분된다(Venable et al.,
2016).
...
> §8.1이 기술하는 승인 안전성 미검정의 실질적 이유이다. 자격 있는 변경이 우연히 부재했던 것이
...
정답 자원이 그래프에 도달하는 정도는 관찰 해상도에 따라 크게 달라진다.

**표 5. 도달성 사다리 — 단일 수치가 아니라 해상도별로 구분하여 보고한다.**
...
태스크와 공유 코어(em·tf·core 23개)를 관찰하며, 두 집합의 합집합이 CQ 28개 전량이다. 이 분리가
규정하는 것은 게이트의 강도가 아니라 귀속이다. 이전 정의에서는 `L3 ⊇ T3` 가 성립하여 T3 단독
...
확보하지 못하였다. 따라서 본 연구는 네 번째 조건을 명시한다.
...
② 비열등 마진을 결과를 확인하기 이전에 동결할 것이다. 두 조건은 충족되었다. 각 층에서 1,188회를
완주하였고 API 실패는 0건이었다. 주 지표(인용 정확도 전체)와 마진 \(\epsilon_{T4} = 0.02\)
(T1에서 승계), 환각률 임계 \(\eta = 0.01\)은 개봉 이전 커밋에 고정되어 있다. 그리고 실제 판정이
1회 수행되었으며, 그 결과는 실패였다. 그럼에도 본 연구는 T4를 승인식에 편입하지 않는다. 단 한
...
4종을 준비하였으나 표현 간 비교는 실행하지 않았다. 주 분석은 청구항 전용 1종만을 사용한다.
...
표 8의 일곱 구성(B0·B2·B3·B4·B5와 P0★·P1)뿐이다.
...
후보 집합을 확대하지 않는다. 이 설계 선택이 §7.2가 진단하는 천장의 원인이다.
...
injection). 본 연구는 결함 12종을 설계하였다. 그중 열 종은 단일 태스크 내부에서 종결되는 결함이다.
신선도·구조·논리·CQ 기능·의미 정렬·계층 평탄화·판단 문맥 치환·메타데이터 삭제·시간 누출·qrel 누출이
여기에 해당한다. 나머지 두 종은 교차 결함이다. 유사 개념의 동의어 오병합과 `Process`/`SubProcess`의 공유 계층
역전이 그것이다. 두 결함은 검색에 무해하거나 유리할 수 있으면서 다른 태스크의 CQ를 훼손한다. 각 유형은 강도 1%·5%·10%로 3회 반복한다(108 인스턴스). 본 연구는 검출
...
본 연구는 정밀도를 주 지표로 사용하지 않는다. 정밀도는 인용되지 않은 문헌 전부를 비관련으로
단정할 것을 요구하지만, 본 연구의 정답은 부분 정답이므로 그러한 단정이 성립하지 않는다. 동일한
이유로 bpref도 사용하지 않는다(§2.1). 대체 관례로 계산한 값은 명칭만 같은 다른
값이기 때문이다([S5](../supplementary/S5-submission-full-v2.md)). nDCG@20은 이진 이득으로
...
본 연구는 P2에서 계층을 하나씩 제거하여 각 계층의 기여를 측정한다.
...
본 논문의 수치는 그래프에 실재하는 링크로 산출되었으므로 판정에는 영향이 없다. 둘째, 하이브리드
결합 산출물의 바이트 수준 재현성이 확보되지 않았다. 기록 순서가 프로세스마다 달라 파일 해시가
달라지지만 순위 자체는 동일하다. 훼손된 것은 판정이 아니라 해시 기반 재현성 검사이며, 이 산출물에
한하여 내용 동등성으로 검증한다.
...
항목을 앞에 배치한 결과이다. 에피소드마다 지위 또한 상이하다. EP1은 관측 사실이고, EP2와 EP4는
확증이며, EP3은 별도 사전등록 아래의 판정이다.
...
아니라 현재 T-Box에서 관측되는 데이터셋 속성이다. 기능 검증에서 G0는 CQ 27/28을, G1·G2는 28/28을
통과한다. 그러나 이 결과는 질의 경로와 비공집합 응답의 존재를 의미할 뿐, 세 태스크의 정확도가
검증되었음을 의미하지는 않는다.

표현 범위와 검색 준비도는 동일하지 않다. 인용된 선행기술 중 그래프에 노드로 존재하는 비율은
95.3%이다. 그러나 도메인 의미 관계로 연결되는 비율은 54.6–70.5%이며, 분류코드까지 포함해야
95.3%에 도달한다(§4.3). 또한 이 도달성 사다리는 언어에 따라 다르게 나타난다. 후보 문서의 개념링크 보유율은
...
G0·G1·G2 사이의 통과율 변동은 온톨로지의 개선이 아니라 A-Box의 충전에 따른 결과이다(세 T-Box가
동일하기 때문이다). 그러므로 본 절의 수치는 세대 안전성의 증거가 아니며, 그 명제는 본 논문에서
검정되지 않았다(§8.1).

## 6.2 EP2 · 게이트 판별력 — 홀드아웃 확증

게이트의 판별력은 확증 점검이 아니라 설계의 근거에 해당한다. 사전등록한 형태는 다른 태스크를
...
한정되는지, 곧 특이성은 아직 검정되지 않았다. ④ 형식층 L2에는 검출 표면이 사실상 존재하지 않는다.
T-Box에 서로소 제약이나 카디널리티 제약이 없어 타입 모순을 주입하여도 논리적 모순이 성립하지 않기
때문이다.

## 6.3 EP3 · 자원만 교체하였을 때 — 게이트가 실제로 거부한 사례
...
차단한 사례이며, §1이 정의한 과제 의미 회귀의 실제 관측에 해당한다.
...
패널 A의 기준선 네 행에서 Δ nDCG@20이 비어 있는 것은 해당 값을 보고하지 않았기 때문이다. 첫
분할의 보조 지표 비교는 사전등록된 두 제안 구성에 대해서만 산출하였으며, 빈칸을 사후에 채우지
않는다.
...
**그림 1.** 검색 유용성의 증분. 확증 분할에서 의미검색을 추가한 이득은 유의하지 않았으며(+0.0189,
...
**그림 2.** 시스템 × 지표. (a) 주 지표 family Recall@100, (b) 보조 지표의 B3 대비 차이와 95%
신뢰구간. 온톨로지 재순위화는 검토 깊이 100 이내로 더 많은 알려진 양성을 회수하지만, 최상위 20위의
정렬 품질은 개선하지 않는다.

### 6.4.2 가설 판정

확증 점검은 검색 유용성과 계층 특이성 둘이며, 두 분할의 판정은 §5.7의 표 4에 분할별로 제시되어
있다. 본 절에서는 그 판정에 이르는 근거만 다시 검토한다.
...
게이트 실행 사례에 관하여 두 가지를 함께 보고한다. 첫째, 게이트에 투입한 델타는 하나가 아니다. 주
사전지정 시스템 P0★도 동일한 게이트에 투입하였으며 결과는 승인이다. 다만 그 여유는 크지 않다. T1 하한은 −0.0139로 마진 −0.02를 근소하게 상회하였다. T2에서는
외국어 정답 포함 집단에 실제 하락 +0.0118이 관측되었으나 δ 미만이므로 통과하였다. 비열등성은 우월성과 다른 개념이므로 P0★의 우월성 미달과 이 통과는 모순되지 않는다. 본
연구는 유리한 델타만 선택적으로 투입하지 않기 위하여 두 판정을 결과 확인 이전에 함께 보고하도록
동결하였다. 둘째, T2의 신뢰집단은 충분하지 않다. `n≥20`을 충족하는 집단이 언어축 2개, 거절근거축
2개, 공정군축 1개뿐이기 때문이다. 따라서 공정군의 안전성은 사실상 단일 집단의 관측이다(§4.5의
보수성 명시와 대응된다). 본 연구는 이를 근거로 공정군 전반의 안전성을 주장하지 않는다.
...
결합 신호에서 발생한다. 둘째, A7(전체 온톨로지 제거)의 +0.0534는 표 8의 P1 대 B3 차이와 동일한
수치이지만 비교 가족이 다르다. 표 8에서는 사전등록된 주 비교이므로 보정 없이 제시하고, 본 절에서는
...
**그림 3.** 계층별 기여와 Holm 보정. 양수는 해당 계층을 제거하였을 때 성능이 저하됨, 곧 그 계층의
...
### 6.4.5 운용 효율 (탐색적) — 주 지표의 이득과 검토 건수의 감소는 일치하지 않는다
...
확대하지 않기 때문이다. 그러므로 이 지점에서 기대할 수 있는 이익은 원리적으로 동일한 문헌을 더 낮은
검토 깊이에서 회수하는 것이며, 더 많은 문헌을 회수하는 것이 아니다. 본 절의 값은 탐색적 기술통계이며,
...
**그림 4.** 검토 깊이별 회수 곡선. 목표 회수에 도달하지 못한 질의는 제외하지 않고 상한+1건으로
...
선행기술 검색에서 Recall@K를 검정하는 것은 그중 한 태스크의 **과제 검증 깊이**를 측정하는
주장이다(§2.4). 이 구분은 일부 CQ의 통과만으로 모든 과제의 효용을 주장하는 과잉 일반화를 차단한다.
CQ와 검색 평가는 서로를 대체하지 않는다. 동시에 CQ는 진화 감시에서 새로운 역할을 획득한다. 정량
평가가 불가능한 태스크의 기능 보존을 검사하는 회귀 스위트가 그것이다(T3). 끝으로 지식그래프 준비도는
다중 해상도로 보고해야 한다. 노드 도달성 95.3%라는 단일 수치는 의미 검색 준비도를 보장하지
않는다(54.6–70.5%).
...
교체가 발생할 때마다 전체 수작업 검토를 수행할 필요가 없다. 그 대신 L0–L3와 동결 검색 회귀 세트,
세 CQ 스위트를 CI에서 실행할 수 있다. 하위집단별 하락을 따로 확인하고(T2) 타 태스크 CQ 통과율을 버전별로
...
온톨로지의 품질을 향상시키면 검색 성능이 향상된다는 명제는 자명해 보인다. 그러나 본 연구에서
관측된 것은 그와 반대되는 현상이다. 동일한 불일치가 서로 다른 세 층위에서 세 차례 관측되었다.
...
**(a)** 재순위화의 천장을 해소하는 온톨로지 기반 후보 생성·질의 확장, **(b)** 상위 정밀도를 겨냥한
...
## 7.9 경쟁 설명 — 구분하지 못한 것을 구분할 실험의 명세로 제시한다

§6.3의 관측, 곧 자원을 2.4배 개선하였음에도 검색 성능이 저하된 현상에는 다섯 개의 경쟁 설명이
존재하며 본 실험은 이들을 구분하지 못하였다. 본 연구가 이를 한 줄의 한계 진술로 처리하지 않는 이유는
§3.3의 문턱 ②가 배제하지 않았음의 명시를 요구하기 때문이다. <!-- style-ok: 경쟁 설명 열거 -->
...
코딩 시트·표본·프롬프트는 코딩 개시 이전 커밋에 동결하였고, 판정 규칙도 결과 확인 이전에
확정하였다.
...
실험이다(§6.3). 상류 교정이 본 연구 최초의 T-Box 술어 델타를 발생시켜 비교 자격이
성립하였다. 이어 하류에 텍스트–개념 적용기를 구현하자 문서집합을 고정한 채 온톨로지만 교체한 두
팔이 최초로 성립하였다. 실측과 판정의 전문은 §6.3에 제시되어 있다. 자원 지표는 향상되었고 교체 대상 구성(P1)의
회수는 저하되었으며, T1 실패로 승인이 거부되었다(Accept = 0). 따라서 최종 진술은 다음과 같다.
승인된 변경의 안전성은 본 논문에서 미검정이며, 자격 있는 델타가 최초로 발생하였을 때의 1회
검정에서는 지지되지 않았다. 다만 그 거부가 자원의 결함에 기인하는지 하류 점수식의 결함에
기인하는지는 구분하지 못하였다.
...
신규 문서에는 적용되지 않는다. 부기하면 질의 길이는 이 문제의 원인이 아니다. 이미 장문 대 장문의
매칭이기 때문이다. 이 제약은 §6.3에서 해소된 상태로 한 차례 측정되었다. 링킹 단계를 결합하여
문서당 개념을 1.545에서 3.779로 증가시켰을 때 온톨로지 단독 팔의 회수는 증가하였고 교체 대상
구성의 회수는 저하되었다. 그 원인은 미구분이다.

세 번째 제약은 단일 언어 검색 파이프라인이다. 질의는 전량 한국어이고 후보 코퍼스는
다국어이다(한국어 39,246 · 영어 1,189 · 일본어 117). 알려진 양성의 41%는 비한국어이다(전체 qrel
2,321건 기준 · 확증 분할 479건으로 한정하면 29.0% · 두 수는 분모가 다르므로 혼용하지 않는다).
따라서 본 논문이 보고하는 Recall은 다국어 선행기술 전체가 아니라 사실상 한국어 하위풀에 대한
회수에 가깝다. 일반화 시에는 개선이 관측된 범위가 한국어 정답임을 함께 진술해야 한다(§6.4.4).

## 8.3 내부·외적·통계적 타당도

질의 인용 간선, qrel 파생 개념, 미래 분류 정보가 잔존하면 누출이 발생한다. 본 연구는 자동 누출
검사와 시점 유효 특징만 사용하는 주 분석으로 이를 통제한다. 주 데이터가 한국 반도체 특허와 특정 거절
표본에 집중되어 있으므로 다른 관할·언어·기술 분야로의 일반화는 검증되지 않았다. 하위집단의 크기가
...
스위트의 해상도도 미확정이다. 결함 주입에서 관측된 T3의 검출력 한계는 CQ가 존재 여부만 검사하고
...
편입했다. 사이드카 CQ는 시험 대상 그래프에 무반응인 상수항이므로 판정 분모에 포함하면 실패를 희석하기
때문이다. 이 결정은 결과 확인 이전에 동결하였으며 회귀 테스트로 강제한다.
...
결정적 하한이다. 문자열 대조는 표현이 변형된 정확한 근거를 놓치고 문자만 일치하는 인용을 통과시키므로,
사람 표본 검증 이전에는 미측정 값으로 해석해야 한다. 보조 지표 둘(환각률·근거 문장 일치)은 두
표본에서 방향이 상반되게 나타났다.
...
담당한다. 승인된 변경의 안전성은 본 논문에서 미검정이다.
