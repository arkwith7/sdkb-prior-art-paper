...
판정과 함께 기록하는 항목이 하나 더 있다. 자원 스냅샷이 변경되어도 코퍼스·개념 축·그래프의
서명이 그대로이면 그 변경은 파이프라인에 도달하지 않은 것이다. 이 상태에서 산출된 회수의 차이는
측정된 0이 아니라 구성상 0이며, 그 위에서 산출된 승인은 변경이 안전하다는 근거가 되지 못한다.
따라서 각 실행은 그 실행의 자원 델타가 게이트에 도달한 상태인가를 함께 기록한다. 기록되는 상태는
셋이다. 동일한 파이프라인 서명이 다른 스냅샷에서 산출된 동결 기록이 있으면 비가시로 남긴다.
그러한 기록이 없으면 비가시의 증거 없음으로, 서명을 산출할 수 없으면 판단 불가로 남긴다. 가운데
상태는 델타가 관측되었다는 뜻이 아니다. 이 항목은 승인식에 포함되지 않으며 판정을 변경하지
않는다. 기록의 목적은 그 승인이 무엇을 근거로 한 승인인가를 남기는 데 있다.
...
동결의 범위는 판정식의 입력 전량이다. 비열등 마진과 하위집단 한계, 주 지표, 비교 구성과 분할,
하위집단 축 셋, 역량질문 스위트 넷이 결과 확인 이전에 문서로 고정되었다. 신뢰구간의 재표집
횟수와 시드, 봉인 평가셋과 시점·패밀리 분할의 해시도 함께 고정하였다. 후보 모집단은 해시로
고정하고 재조립하지 않았으며, 순위 함수의 가중 격자도 동결 대상에 포함된다. 그러므로 두 실행
사이에서 변한 것은 자원 번들 하나이다(§4.5).
...
계층 기여 점검은 기각되었다. 곧 어느 계층이 기여를 산출하는지는 절제로 구분되지 않았다. 나머지
열두 행과 그 판정의 상세는 [S9](../supplementary/S9-retrieval-evaluation-detail.md)에 있다.
