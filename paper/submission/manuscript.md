...
이 사건의 경로이다. 이 사건의 수치는 §5.2가 보고한다.
...
독자는 3–5장에서 하나의 **관통 예시(running example)** 를 반복하여 만난다. 관통 예시는 데이터셋에
등재된 거절특허 하나를 자원 구조, 평가 설계, 판정의 순서로 다시 읽는 설명 장치이다. 이 사례의
RDF를 **예시 그래프(example graph)** 라 하며, 실측 그래프에서 발췌한다. 또한
본 논문의 **스킴 경로(skim path)** 는 장·절 제목과 그림 캡션 첫 문장, 예시 첫 문장만 이어 읽는
시각적 논증 골격을 뜻한다. 사례는 기제를 예시하고 실제 판정은 사전등록 집계가 담당한다.

이후의 구성은 다음과 같다. 2장은 선행연구와 연구 공백을, 3장은 산출물과 설계·평가 절차를 다룬다.
4장은 평가 설계를, 5장은 다섯 에피소드가 산출한 발견 넷을, 6장은 논의와 설계지식, 한계를 다루며
7장이 결론이다.

![그림 1. 자원 변경 한 건이 승인 절차를 지나며 받은 판정 — 변경, 자원 지표, 형식 검증, 태스크 조건, 승인 결과.](../figures/concept_overview.svg)

**그림 1.** 자원 지표는 개선되었고 형식 검증 네 층을 전부 통과하였으나, 사전 지정 비열등
기준을 충족하지 못하여 그 변경은 거부되었다. 위에서 아래로 하나의 사건을 따라 읽는다. 왼쪽
열은 단계와 그 단계가 속한 산출물이며, 자원 ART-1에서 승인 게이트 ART-2를 거쳐 평가환경 E1의
지표에 이른다. 오른쪽은 각 단계에서 관측된 값이다. 태스크 조건 행의 아랫줄은 나머지 두 조건이
충족되었음을 밝힌다. 이 판정의 수치와 그 조건은 §5.2가 보고한다.
...
> 삼는다. 2021년 출원된 이 특허는 심사관이 인용한 선행문헌 여섯 건 때문에 진보성 위반으로
> 거절되었으며, 데이터셋에 A-Box로 등재되어 있다. 거절이유통지서의 사실 가운데 아홉 개를
> 그래프에서 그대로 발췌하면 다음과 같다.
>
```turtle
...
> 위쪽 다섯 트리플은 선행기술조사 뷰가 읽고, 아래 셋은 전문가 매칭 뷰가 읽는다. 두 뷰는
> `plasma_etch` 공정 노드에서 만난다. 이 노드의 변경이 두 뷰의 답을 함께 바꾸는 위험을
> §3은 구조로 설명하고 §4–§5는 평가와 판정으로 확인한다.

이 특허는 §5.2의 추적 대상과 동일하며, 결과를 확인한 뒤 고른 것이 아니라 동결한 규칙이
지정한 것이다(§4.5). 따라서 §3의 구조 설명과 §5의 판정은 같은 인스턴스 위에서 읽힌다. 다만
발췌에는 제약이 둘 있다. KIPRIS Plus 원문의 재배포 범위가 제한되므로 초록과 청구항 본문은
싣지 않는다. 또한 한 특허의 이웃 전체는 설명용 지면을 초과하므로 두 뷰가 만나는 경로만 남긴다.
발췌는 생성기가 그래프에서 확인한 뒤 출력하며 손으로 옮겨 적지 않는다.
...
구분된다(Venable et al., 2016). 본 연구의 순환은 문제 식별, 설계·개발, 1차 평가, 설계 개선,
재평가, 실제 개정 평가, 이식 판정, 설계지식 도출의 순서를 따른다. 초기 기각과 그 뒤의
...
![그림 4. 승인 절차의 순서와 각 항의 미충족 시 처리.](../figures/concept_gate_flow.svg)

**그림 4.** 형식 검증 네 층을 통과한 변경도 태스크 조건에서 멈추며, 각 항은 미충족 시 뒤를 실행하지 않는다. 왼쪽 열은 승인 절차의 순서이며 위에서 아래로 읽는다. 오른쪽
열은 각 항이 미충족일 때의 처리이다. 이 절차가 통제된 자원 교체에서 실제로 낸 판정은 그림 1이
보이며, 그 수치는 §5.2에 있다.
...
세 조건이 각각 무엇을 묻고 무엇으로 판정하는가는 다음과 같다.

| 조건 | 묻는 것 | 지표 | 판정 통계 | 임계 | 실증 |
|---|---|---|---|---|---|
| T1 | 검토 깊이 100의 회수가 저하되었는가 | family-level Recall@100 (§4.5) | 질의 단위 쌍체 부트스트랩 10,000회 신뢰구간의 하한 | \(\epsilon\) = 0.02 | §5.2 · §5.3.1 |
| T2 | 특정 하위집단에 국소 회귀가 있는가 | 집단별 Recall@100 | 집단별 하락의 최댓값 · 결정론적 규칙 | \(\delta\) = 0.05 | §5.2 · §5.3.2 |
| T3 | 다른 태스크의 질의 경로가 회귀하는가 | 스위트별 역량질문 v2 통과율 | 세대 간 통과율 비교 · 결정론적 규칙 | \(\tau\) = 0.05 | §5.2 · §5.4 |
...
불변이었고 Ontology-only 구성만 27% 이동하였다(표 5). 구성이 하나뿐이라면 이 역할 분담은
성립하지 않는다. 노출 대조군만 관측하였다면 같은 교체는 회수 향상으로 읽혔을 것이나, 하류
센서에서는 저하가 관측되어 승인이 거부되었다(§5.2). 자원 변경의 승인 여부는 노출 대조군이
아니라 하류 센서에서 결정된다. 밀집 기준선 B2는 Titan Embed v2 하나이다.
...
| 검색 유용성 점검 | 첫 분할 부분 지지 · 두 번째 분할 미지지 | 표 7 · S9 |
...
성능 조건 T1이 거부하였다. 그림 1이 이 변경의 판정 경로를 단계별로 보이며, 그림 7의 왼쪽 두
패널이 자원 지표의 상승과 검색 지표의 저하를 사전등록 집계로 보인다. 이 진술의 범위는 자격을 갖춘 실제 변경 1건이며, 차단된
...
이 판정이 질의 하나에서 어떤 모습인지는 표 6이 보인다. 사례는 동결한 규칙이 지정한 것이며, 그
규칙은 온톨로지 결합 구성에서 순위가 밀린 정도가 중앙값에 가장 가까운 질의를 고른다. 선정된
거절특허는 진보성 위반으로 거절되었고 심사관은 선행문헌 여섯 건을 인용하였다. 그 가운데 한 건에서
텍스트 하이브리드 구성의 순위는 자원 교체와 무관하게 유지되었으나, 온톨로지를 결합한 구성은 교체
이후 같은 문헌을 더 아래로 내렸다. 온톨로지 단독 구성은 이 문헌을 회수하지 못하였다. 질의와 이
문헌이 공유하는 개념이 사실상 없기 때문이다.
...
같은 교체를 198개 질의 전체에서 문헌 단위로 세면, 상위 100위 안에 심사관 인용을 하나 이상 보유한
질의는 136개에서 133개로 감소한다. 이 계수는 주 지표와 단위가 다르므로 판정의 근거가 아니며, 주
지표의 저하가 검토 화면에서 어떤 크기인지를 가리키는 환산이다.
...
표본 변동으로 저하가 과소평가되는 경우를 배제하기 위함이다. 두 수를 바꾸어 인용하면 판정의
강도가 달라진다. 이 구간은 0을 포함하지 않으며 상한도 −0.0053으로 음수이나, 비열등 마진
−0.02를 가로지른다. 그러므로 이 표본이 확정한 것은 저하폭이 마진을 초과하였다는
사실이 아니라, 마진 안에 있음을 보이지 못하였다는 사실이다.

이 사례는 층간 지표 불일치를 그대로 보여준다. 자원 층의 수량 지표는 방향이 하나로 일치하였다.
코퍼스에 연결된 개념 어휘가 늘고 문서 한 건당 연결 개념이 2.4배가 되었다. 이 값들은 T-Box
클래스 증가가 아니라 기존 문서에 대한 개념 매핑의 확대를 나타낸다. 온톨로지를 수정하는 주체가
관찰하는 것이 바로 이 지표들이므로, 자원 층만 본다면 이 변경은 명백한 개선이다. 그러나 한 층
아래의 검색 지표는 반대 방향으로 움직였다. 곧 자원 층의 지표는 이 변경에 관한 한 다음 층의
성능을 대표하지 못하였다(§6.1).
...
예시한다. 곧 공유하는 개념이 변별력을 갖지 못하는 질의에서는 개념 항이 심사관 인용을 오히려
밀어내며, 그 분해는 같은 표의 주석에 있다. 본 에피소드는 봉인 분할에 대하여 사전등록된 확증 평가를 두 차례
...
않았다. 주 구성은 유의에 이르지 못하였고 저중첩 조건은 반증되었다(표 7 · S9). 첫 사전등록이
...
**두 판정을 읽는 전제.** 정답이 부분적이므로 §4.5의 두 점검을 수행하였고 그 뒤에도 차이는
유지되었다. 효과를 확실하다고 기술하기 위한 조건 다섯 가운데 충족된 것은 하나이므로 본 연구는
그 표현을 사용하지 않는다. 세 전제의 전문과 잔여 취약성의 정량은
...
설명은 구분되지 않는다. 곧 온톨로지 전체는 기여하나 어느 축이 그 기여를 산출하는지는 구분하지
못하였다. 절제 결과가 지지하는 주장의 범위와 거절근거 축의 자원 한계는
[S9](../supplementary/S9-retrieval-evaluation-detail.md)에 있다.
...
우월성은 이 값으로 지지되지 않는다. 회수 분해와 도달 상한의 정량은
[S9](../supplementary/S9-retrieval-evaluation-detail.md)에 있다.
...
이 판별력의 범위도 함께 보고한다. 검출률은 분포 검사 임계에 민감하며 임계별 전환점은 표 8에,
동결 격자 세 값의 검출 수와 검정 결과는 [S2](../supplementary/S2-fault-injection-v09.md)에 있다.
사전 지정 임계에서도
...
이라 하며, 홀드아웃 모델의 관찰면은 개발 모델보다 좁았다. 곧 이 결손은 질문의 결함이 아니라
홀드아웃 인스턴스의 충전 범위이다. 둘째, 21건 전량에서 형식 검증 네 층과 교차 태스크 조건이 모두
미검출이었으므로 이 표본은 조건 T3을 다른 층과 비교할 수 있는 표본이 아니었다. 원인은 술어의
방향이며, 두 모델이 포함관계를 서로 다른 술어로 표현한다. 곧
결함 명세는 대상 자원의 모델링 관습에 맞추어 재정의되어야 하며, 이 관측을 게이트가 교차 결함을
놓쳤다는 진술로 옮기지 않는다. 관찰면 계수와 술어별 주입 후보 수는
[S8-A](../supplementary/S8-second-domain-port.md#s8-a--릴리스-계보와-운용-비용)에 있다.
...
![그림 8. SDKB 홀드아웃과 제2 자원에서 관찰된 검출 범위, 그리고 그 관측에서 나온 교훈 셋.](../figures/concept_detection_port_boundary.svg)
...
아래 띠의 교훈 셋은 확립된 원리가 아니라 후속 연구가 검정할 가설이며, 근거와 약화 조건은
§6.3에 있다.
...
취한 규범적 선택이다(§3.5). 표 8은 각 판정이 전환점에서 얼마나 떨어져 있는가를 보고한다. T1의
