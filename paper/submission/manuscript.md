...
회수이며, 주장의 범위도 그 집합으로 한정한다(§6.4). 판정이 불완전한 평가에는 미판정에 덜
...
같은 층의 다른 단위인 검토 건수를 가리킨다. 관측 ①은 §5.1, ②는 §5.4.3, ③은 §3.5.1·§6.4에
제시하며 해석은 §6.1에 있다.
...
| **EP4** | **검색 이득의 범위와 경계** | 온톨로지 보강이 강한 텍스트 기준선을 **개선하는가, 어디까지인가** | 봉인 분할에 대한 사전등록된 확증 평가 — 모든 접근을 열람 원장에 기록 (비중복 확증 분할 둘) | 확증 + 탐색적 진단 | §5.4 |
...
지위를 갖는다. 자원 변경의 효과를 판별하려면 문서집합·모델·가중치를 고정한 채 자원 번들만
교체해야 하며, 이는 본 연구가 평가를 설계하며 둔 방법론적 요구사항이다. EP2(§4.4)에서 온톨로지는
시험 대상이고, EP3(§5.1)에서는 문서집합·설정·가중치를 고정한 유일한 변인이며, EP4(§4.1–4.3 · §4.5)에서는 순위 함수의 입력 특징이다. §4.4의 A8에서는
...
온톨로지에 등재된 조건에서 산출되었으며, 이 제약의 영향과 해소 방법은 §6.4에 있다. 주 분석의 질의 단위는 거절 특허 1건이며, 질의 본문은 독립항
...
안전성**(§6.4) · **계층 기여**(§5.4.2)이며, 확증 점검이 아니라 설계 근거로 다루되 판정은 그대로
...
설계지식의 승격 규칙도 결과 확인 이전에 정하였다. 이 이식은 T1·T2를 포함하지 않으므로 승인
층에 관한 교훈의 근거를 늘리지 않는다. 근거 확대의 대상은 교차 태스크 감시(§6.3 교훈 ②)뿐이다(전문 [S8](../supplementary/S8-second-domain-port.md)).
...
이 절로 변경되지 않으며, 승인 안전성에 대한 판정은 §6.4에 있다.
...
<!-- style-ok: 클래스 목록 열거 -->
저장소의 TTL에는 세 뷰의 앵커 클래스가 모두 존재한다. 전문가 매칭의 `Problem`·`FailureMode`·
`Skill`·`Expert`, 선행기술조사의 `Claim`·`ClaimFeature`·`PriorArtJudgment`와 인용·판단 관계,
기술예측의 `TechnologyNode`·`Scenario`·`RealOption`과 `filingDate` 시간축이 그것이며, 뷰별 전량은
...
통과하므로 감사 전량 31개 기준 G0는 30개이다(§3.4 표 4).

표현 범위와 검색 준비도는 동일하지 않다. 인용된 선행기술의 그래프 도달성은 무엇을 연결로
인정하는가에 따라 크게 달라지며, 관찰 수준별 값은 §3.3에 있다. 이 도달성은 언어에 따라서도 다르다. 후보 문서의 개념링크 보유율은 한국어 99.2% ·
영어 69.6% · 일본어 0%이고 분류 보유율만 세 언어 모두 100%이다. 곧 언어중립 개념 IRI는 T-Box
수준의 속성이며, A-Box 수준에서는 비한국어 문서에 개념이 적게 결합되어 있다. 이 비대칭이 §5.4.3을
해석하는 전제가 된다.

한정요소 자원의 계수와 판단 연결 표본의 도달성은 §3.2·§3.3에 있으며, 두 값은 청구항 수준 평가의
실행 가능성을 지시한다. 교차 태스크 CQ 통과율은 저하되지 않았으며 누적 waiver는 0회이다.
...
절의 수치는 세대 안전성의 증거가 아니다(§6.4).

## 5.4 EP4 · 검색 이득의 범위와 그 경계

본 에피소드의 결론은 이득이 관측된 범위이다. 온톨로지 재순위화는 사전에 기대하였던 세 자리에서
개선을 보이지 않았다. 그 셋은 어휘 불일치 질의와 상위 정렬, 그리고 검토 효율이다. 개선이 관측된
자리는 깊은 회수 하나이며, 그 이득도 운용 단위로 환산하면 소멸한다. 본 연구는 이러한 결과가
릴리스 이전에 검출되어야 한다는 것을 요지로 삼는다.
...
해소되며, 두 점검의 전문은 S5에 해소 명세는 §6.4에 있다.
...
1건이며, 그 범위를 넘는 일반화는 §6.4의 후속 가설로 남긴다. 형식 검증 네 층과 조건 T3은 프로파일 교체만으로
...
동결하고 수행한 판정은 실패하였으며(§3.5.1 · §6.4), 그 원인이 전달의 부재인지 검정력의 부족인지는
구분하지 못하였다.
...
층의 결과를 측정해야 한다는 것이다. 따라서 본 연구는 온톨로지 내부의 품질과 하류 태스크의 성능을
서로 다른 층에서 평가하며, 두 층의 지표를 대체 관계로 두지 않는다. 같은 구분에서 CQ는 정량 평가가 불가능한 태스크의 기능 보존을
...
승인한다. 근거는 §5.1의 판정이다. 자원 지표를 전부 향상시키고 형식 검증 L0–L3를 전부 통과한
변경이 성능 조건 하나에서 거부되었다(Accept = 0). 이 형태는 게이트를 설계하는 시점에 이미
기술되어 있었고 실제 거부로 지지되었다. 이 배치가 없었다면 §5.1의 변경은 자원 지표의 전량 향상과
형식 검증의 전량 통과를 근거로 승인되었을 것이다. 형식 검증만으로 하류의 비회귀가 예측되고 태스크
조건이 승인 결정을 한 번도 바꾸지 않는다면 이 교훈은 약화된다. 근거의 폭은 자격을 갖춘 자원 델타
1건이다.

**교훈 ② · 교차 태스크 감시.** 공유 T-Box에서는 주 태스크의 성능만으로 변경을 승인하지 않는다.
근거는 홀드아웃 결함주입이다. 형식 검증과 검색 성능 검사가 모두 놓친 결함을 T3이 단독으로 검출한
사례는 45건 가운데 12건이고(단측 *p* = .0001 · §5.2), 정상 델타 27건에서 위양성은 0건이었다. 주
태스크 검사의 검출률이 T3과 같고 T3의 단독 검출이 0이면 이 교훈은 약화된다.

이 교훈의 보조 근거는 절제에서 관측되었으며 그 지위가 앞의 근거와 다르다. 한 태스크를 위하여
설계한 계층이 다른 태스크의 성능에 관여할 수 있음이 관측되었다. 다만 판정은 확정적이되 원인은
구분하지 못하였고, 관측은 하나의 확증 분할과 이 릴리스의 모듈 배치에 조건부이다.
...
구분한 실험이 없으므로 원인은 확정하지 않으며, 근거는 S5에 예측은 §6.3에 있다. 두 번째 확증
분할에서 동일 절제의 값은 정확히 0.0000이므로 이 관측은 재현되지 않았다(§5.4.2). 두 표본이 갈린
원인도 구분하지 못하였다.

이러한 불일치는 교차 태스크 조건의 근거를 무효화하지 않지만 근거의 우선순위는 바꾼다. 따라서 이
교훈의 주된 근거는 결함주입 결과이며, 음성 대조군 결과는 두 표본의 불일치를 포함한 보조 근거로
한정한다. 이 관측의 현재 등급은 **설계 진단**이다. 제2 공학 온톨로지의 판정으로 근거가 늘지도
않았다. 판정이 가능한 결함 12건에서 T3의 단독 검출이 0건이었기 때문이다(§5.5). 다만 이 0은 위
약화 조건이 성립한 결과가 아니다. 그 표본에서는 형식 검증 네 층과 교차 태스크 조건이 모두
미검출이어서 검출률 비교의 대상이 성립하지 않았다. 0건의 분모는 21건이 아니라 12건이며, 나머지
9건은 결함이 주입되지 않아 판정에 이르지 못하였다.

**교훈 ③ · 후보생성 분리.** 재순위화는 후보 풀 밖의 문헌을 회수할 수 없으므로 후보 생성과
구분하여 평가한다. 근거는 검토 깊이에 상한을 두지 않았을 때 세 시스템의 미도달 질의 비율이 완전히
동일하다는 관측이다(정답 절반 26.8% · 완전 회수 55.1% · §5.4.3). 재순위화만으로 그 비율이
감소하면 이 교훈은 약화된다.
...
전달 검증이다. 이는 검색 지표의 개선을 검토 효율이나 생성 답변의 품질로 자동 일반화하지 않는다는
명제이다. 본 연구의 판정은 이 방향을 지지하나, 실패의 원인이 전달의 부재인지 검정력의 부족인지는
구분하지 못하였다(실패의 폭은 표 11). 검색 지표의 개선이 검토 건수나 생성 품질로 일관되게
전달되면 이 가설은 약화된다. 둘째는 **이식 계층 분리(port-layer separation)** 이다. 승인 게이트를
다른 자원으로 옮길 때 층 구조와 판정식, 실행으로 이루어진 기제는 이전된다. 반면 델타 제약과 결함
정의, 역량질문으로 이루어진 명세는 대상 자원의 표현 관습과 인스턴스 관찰면에 재접지하여야 한다. 근거는 제2 자원에서 관측된 세 자리이다. 동결한 델타 제약은 정의역과 치역을 스키마
어휘로 선언할 것을 요구하나 대상 자원은 제약 언어로 선언한다. 그 결과 릴리스 계보 10판정 가운데
8판정이 이 규칙에서 미충족이었다. 동결한 결함 정의는 포함관계를 한 방향의 술어로 기술하나 홀드아웃 모델은
반대 방향을 사용하므로 21건 가운데 9건이 주입되지 않았다. 역량질문 15개 가운데 홀드아웃에서 행을
산출한 것은 7개였다(§5.5). 이식 사례가 1건이므로 근거의 폭은 그만큼으로 한정한다. 표현 관습과
인스턴스 구성이 다른 자원에 명세를 그대로 옮겼는데도 검출력이 유지되면 이 가설은 약화된다. 이
구분은 앞의 결과를 다시 읽는 방식도 바꾼다. 제2 자원의 검출 0을 게이트의 실패로 읽으면 기제와
명세가 구분되지 않는다.
