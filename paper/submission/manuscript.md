...
구분된다(Venable et al., 2016). 본 연구의 순환도 한 차례로 끝나지 않았다. 표 1은 그 순환을
설계과학연구의 단계에 대응시키고 각 단계가 수행된 절을 함께 제시한다. 1차 평가에서 게이트의
판별력이 기각된 사실과 그 뒤의 재설계가 이 표의 중심에 있으며, 이 반복설계의 이력이 교훈의 근거를
이룬다. 설계지식의 승격 기준은 결과를 확인하기 이전에 동결하였고 그 내용은 §6.3에 있다.
...
세 측면에서 구분된다(표 2). 다섯은 모두 통제된 조건에서 수행하였고 실제 운영 환경에서의 현장
...
**결손의 명세.** 결손은 모두 아홉이며 승인 게이트에 관한 여섯(①–⑤·⑨)은 아래 표에, 검색 층
평가에 국한된 셋(⑥–⑧)은 S9에 있다. 주장의 범위를 가장 크게 제약하는 것은 승인 안전성의
미검정(②)이다.

**표 9. 검증 강도와 일반화 가능성의 결손 여섯 — 해소 방법은 전부 새 사전등록의 대상이다.** 결손 ⑥–⑧은 검색 층 평가에 국한된 결손이며 [S9](../supplementary/S9-retrieval-evaluation-detail.md)에 같은 번호로 있다. 번호는 본문과 보충자료를 잇는 고정 식별자이므로 다시 매기지 않는다.
