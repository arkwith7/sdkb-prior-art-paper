...
다국어에 특화된 장문 밀집 인코더는 이 두 제약에 걸리지 않으나 비교에 포함하지 않았으며, 그 이유는
성능의 예단이 아니라 동결 규칙이다. 모델은 개발셋을 개봉하기 이전에 동결하는 사전등록 대상이므로,
인코더의 추가는 재측정이 아니라 새 사전등록 아래의 실험에 해당한다. 이 부재의 귀결은 §5.4.3의
교차언어 진단에 직접 미친다. 그 진단이 비교하는 것은 다국어 검색 방법 사이의 우열이 아니라 본
파이프라인 내부의 구성이며, 이 결손의 해소 명세는 §6.5에 있다.
...
| ⑦ | 강한 다국어 기준선의 부재 | 밀집 기준선은 단일 임베딩 모델이며 다국어 특화 인코더는 미평가 (§4.3) | 다국어 밀집 인코더를 기준선에 추가한 재평가 |
