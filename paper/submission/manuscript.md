...
| 자원 지표를 유용성의 대리로 쓰는 관행 | Strathern (1997); Chiu et al. (2016); Thomas & Uminsky (2020) | 어긋남이 상관 분석 수준에서 보고될 뿐 **통제된 사례와 그에 대한 결정**은 드물다 | 자원만 교체한 두 팔에서의 통제된 확인과 **승인 판정**(§6.3) |
...
| **EP2** | **게이트 판별력** | 의도적으로 주입한 결함을 게이트가 **검출하는가**, 정상 변경을 **거부하지는 않는가** | 아직 판정한 적 없는 홀드아웃 결함 · 사전 지정한 세 조건 | 확증 | §6.2 |
...
**표 4. 도달성 사다리 — 단일 수치가 아니라 해상도별로 구분하여 보고한다.**
...
**표 5. 승인식의 각 항과 미충족 시의 처리.**

| 항 | 조건의 내용 | 미충족 시의 처리 |
|---|---|---|
| L0–L3 | 최신 상태인가 · 구조 제약을 충족하는가 · 논리적 모순이 없는가 · 필수 역량질문에 응답하는가 | 즉시 거부 |
| **T1** | 변경 이후에도 **검색 성능이 저하되지 않았는가** | 허용 폭 \(\epsilon\) 을 초과하여 저하되면 거부 |
| **T2** | 전체 평균과 별개로 **특정 하위집단만 저하되지 않았는가** | 한 집단이라도 \(\delta\) 이상 저하되면 거부 |
| **T3** | **다른 태스크**가 응답하던 역량질문에 여전히 응답하는가 | 통과율이 하나라도 저하되면 거부 |
...
**표 6. 사전등록된 평가 점검과 그 판정 — 근거와 전문은 6장·supplementary [S5](../supplementary/S5-submission-full-v2.md).**
...
![그림 2. 시스템 × 지표 — 깊은 회수의 개선과 상위 정렬의 미개선.](../figures/ir_metrics.svg)
...
확증 점검은 검색 유용성과 계층 특이성 둘이며, 두 분할의 판정은 §5.7의 표 6에 분할별로 제시되어
...
![그림 3. 계층별 기여(A1–A8)와 Holm 보정 — 보정 후 유의한 절제는 음성 대조군 A8 하나.](../figures/ir_ablation.svg)
...
![그림 4. 검토 깊이별 회수 곡선 — 검토 깊이 K에 따른 family Recall@K 곡선(로그 축).](../figures/ir_effort_curve.svg)
...
편입하였다. 사이드카 CQ는 시험 대상 그래프에 무반응인 상수항이므로 판정 분모에 포함하면 실패를 희석하기
