...
데이터셋 서술은 공유 T-Box가 게이트에 무엇을 제공하는지를 진술한다. 교차 태스크 조건은 둘 이상의
태스크를 담은 자원을 전제로 하기 때문이다. 검색 평가는 성능 조건이 읽는 지표가 작동하는 측정 도구임을
확인하고 그 지표가 보고하는 이득의 경계를 측정한다. 제2 공학 온톨로지로의 이식은 게이트가 어디까지
이전되는지를 한정한다. 셋 가운데 어느 것도 데이터셋 품질의 주장이나 검색 성능의 우열,
또는 일반적 이식 가능성의 주장으로 제시되지 않는다.
...
보이는 것과, 발견 ②의 실측 근거가 되는 것이다. 검색 유용성은 사전등록된 점검이었고 표 2는 그
