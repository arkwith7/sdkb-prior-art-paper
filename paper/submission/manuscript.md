...
평가한다. 셋째는 **설계지식**이다. 자원 지표가 개선되고 형식 검증을 통과한 변경이 오히려 검색
성능을 저하시킬 수 있음을 통제된 교체로 제시한다. 그 관측에서 층별 검증과 한 층 아래 승인이라는
이전 가능한 설계원리를 도출한다. 나아가 그 게이트를 표현 관습이 다른 제2 공학 온톨로지에 형식 층과
...
4장은 평가 설계를, 5장은 다섯 에피소드의 결과를, 6장은 논의와 설계원리, 한계, 결론을 다룬다.
...
도면 요소를 온톨로지로 규정하였다(Schönfelder & König, 2025). 아키텍처의 모듈화는 생애주기마다
달라지는 요구를 흡수하는 의미 기반 디지털 트윈을 제시하였다(Kosse et al., 2025). 응용 성능의
실증은 언어모델 기반 에이전트가 생성한 안전 위험성 평가를 지식그래프에 적재하였다(Speiser et al.,
2026). 그 정확성은 전문가 평가와 SHACL 검사로 확인되었다. 또한 안전과 공정계획이 지식그래프를
공유할 때 한 도메인의 결정이 다른 도메인에 미치는 영향도 질의로 드러났다(Johansen et al., 2025).

세 방향은 자원이 무엇을 표현하며 응용에서 어떻게 동작하는가를 보이는 반면, 그 자원이 변경될 때
어떤 변경을 수용하고 거부할 것인가의 규칙은 제시하지 않는다. 모듈 구조는 변경을 받아들일 통로를
넓히지만 개별 변경의 수용 여부를 판정하지 않으며, 도메인 사이의 영향에 대한 관측도 승인 조건으로
사용되지 않는다. 이 분야의 검증 논의 역시 구조와 규칙의 준수 여부를 보고하는 데 머문다(Solihin
...
자원으로 옮겨진다는 것은 서로 다른 주장이다. 승인 규칙은 실행 절차만이 아니라 무엇을 위반으로 볼
것인가를 규정하는 제약과 질의를 함께 요구하며, 그 제약과 질의는 대상 자원의 표현 관습에 의존한다.
예컨대 술어의 정의역과 치역을 스키마 어휘로 선언하는 자원과 제약 언어로 선언하는 자원에서는 같은
규칙이 서로 다른 결과를 낸다. 그러나 승인 규칙을 다른 자원에 적용하여 무엇이 함께 옮겨지고 무엇이
재정의를 요구하는지를 실측한 기록은 확인되지 않는다. 본 연구는 이 공백을 다섯째 평가
에피소드에서 다룬다(§4.6 · §5.5).
...
같은 층의 다른 단위인 검토 건수를 가리킨다. 관측 ①은 §5.1, ②는 §5.4.3, ③은 §3.5.1·§6.5에
제시하며 해석은 §6.1에 있다.
...
| 자원 지표를 유용성의 대리로 쓰는 관행 | Strathern (1997); Chiu et al. (2016); Thomas & Uminsky (2020) | 어긋남이 상관 분석 수준에서 보고될 뿐 **통제된 사례와 그에 대한 결정**은 드물다 | 자원 번들만 교체한 두 조건에서의 통제된 확인과 **승인 판정**(§5.1) |
| 공학 정보학의 의미 표현·검증과 응용 | Schönfelder & König (2025); Kosse et al. (2025); Speiser et al. (2026); Solihin et al. (2015); Pauwels et al. (2024) | 표현의 표준화·구조 준수·응용 성능은 제시되나 **변경의 승인 규칙**은 다루지 않는다 | 사용 가능성이 아니라 **변경 수용 가능성**을 판정하고 실제 심사 기록을 제시(§5.1) |
...
| 설계·개발 | 산출물 A1(SDKB)과 A2(T-gate) | §3.1–3.6 |
...
| 실제 개정 기반 통제 평가 | 자원의 실제 개정에 대한 게이트의 거부(Accept = 0) | §5.1 |
...
**동결(freeze)** 하며, 고정은 이후 버전의 개선을 제약하지 않는다(§6.6).
...
변경에도 같은 형식으로 적용된다. 자원 변경에 대한 승인 판정은 §5.1의 1회뿐이며, §5.4.1의 판정은
...
열은 각 항이 미충족일 때의 처리이고, 오른쪽 열은 통제된 자원 교체(§5.1)에서 각 항이 실제로 낸
...
지위를 갖는다. EP2(§4.4)에서 온톨로지는 시험 대상이고, EP3(§5.1)에서는 문서집합·설정·가중치를
...
금지한다. 여섯 항 가운데 계층 경로의 가중 \(w_h\)는 그 격자에서 0으로 수렴하였으므로 계층만 변경하는
델타는 이 점수식에 원리적으로 관측되지 않는다(§6.2). 또한 제안 시스템은 텍스트 기준선의 상위
1,000건을 재정렬할 뿐 후보 집합을 확대하지 않으며, 이 설계 선택이 §6.2가 진단하는 재순위화 상한의
원인이다(계수는 S5).
...
injection). 본 연구가 설계한 결함은 12종이다. 열 종은 단일 태스크 내부에서 종결되며 유형은
...
유형은 강도 1%·5%·10%로 3회 반복하고(108 인스턴스) 정상 변경을 오거부한 비율도 보고한다.
...
절제(ablation)는 부차 구성 P1에서 계층을 하나씩 제거하여 각 계층의 기여를 측정하며, 기준 구성이
P1인 이유는 P2가 실행되지 않았기 때문이다(§4.3). 사전등록한 절제 조건은 여덟이고 본문이 정의하는
것은 둘이다. **A7**은 모든 온톨로지 특징을 제거한 조건이고 **A8**은 전문가 매칭 전용
계층(`Skill`·`ExpertCase`·`Mitigation`)을 제거한 조건이며, 나머지 여섯(A1–A6)의 정의와 결과는
전량 S5에 있다.
...
<!-- style-ok: 지표 목록 열거 -->
보조 지표는 세 질문에 대응한다. 깊은 구간까지의 회수에는 Recall@50과 Recall@500이 답하며, 본
논문은 상위 100건 이상의 구간에서 관측되는 회수를 **깊은 회수(deep recall)** 라 부른다. 상위
정렬에는 **정규화 할인 누적 이득(normalized discounted cumulative gain, nDCG)@20** 과
**평균 역순위(mean reciprocal rank, MRR)@K**, **상위 K 적중 여부(Success@K)** 가 답한다. 검토
비용에는 **목표 회수율 도달 노력(Effort@Recall)** 과 **후보 축소율(Candidate Reduction)**, 그리고
지연시간이 답한다. 생성 단계의 네 지표는 검색 층의 지표가 아니므로 §5.4의 표에 포함하지 않는다.

본 연구는 정밀도와 **판정 비적합 기반 선호도(binary preference, bpref)** 를 사용하지 않는다. 두
지표는 인용되지 않은 문헌을 비관련으로 단정할 것을 요구하나, 본 연구의 정답은 부분 정답이므로 그
단정이 성립하지 않는다(§2.1). nDCG@20은 qrel이 전량 등급 1이므로 이진 이득으로 계산하며, 등급을
사후에 생성하지 않는다. 정답이 부분적이므로 짝지은 비교가 미판정 문헌에 의존하는지를 두 방식으로
점검한다. 상위 100위 미판정 집합의 조성 대조와 판정 전환 최소 건수이며, 어느 쪽도 새로운 판정을
만들지 않는다. 두 점검의 정의와 전량 결과는 [S5](../supplementary/S5-submission-full-v2.md)에
있다.

통계 분석은 다음과 같이 수행한다. 시스템 비교는 동일 질의에 대하여 짝지어 10,000회
재표집(paired bootstrap)하여 95% 신뢰구간을 산출하며, 짝짓지 않으면 질의 난이도의 차이가 성능
차이로 오인될 수 있다. 점추정과 승·패·동은 원표본의 질의별 차이에서 직접 산출하고, 양측 *p*값은
재표집 분포에서 계산한다. 재표집 시드는 개봉 이전에 코드에 고정하였다(§4.1). 검출률의 층간
비교에는 맥니마 검정을, 다중 비교에는 **홀름 보정(Holm correction, Holm)** 을 적용한다. 하위집단
결과에는 질의 수와 정답 수를 함께 보고하며, 표본이 부족하면 결론을 유보한다. 어휘 중첩 집단의
경계도 결과 확인 이전에 동결하였다. 개발셋 197질의의 문자 3-gram Jaccard 분포에서 Q1 = 0.0079를
low-overlap 경계로 고정하였다. 확증 분할에 같은 값을 적용하여 low 27 / high 171로 구분되었다.
이 점수는 순위 함수에 투입되지 않는 **분석용 층화 라벨**이다.

본 연구는 결과를 확인하기 이전에 평가 점검 셋을 동결하였다. 두 확증 분할은 겹치지 않으며 각각
별도로 등록되었으므로 아래 표는 예측과 판정을 분할별로 구분한다.
데이터·온톨로지·shape·CQ·인덱스·모델 버전은 해시로 고정하였고 시드와 lockfile, 분할 식별자 목록을
공개한다. 점검 명칭과 등록 문서의 대응은
...
세 점검 외에 설계 근거로 함께 등록한 항목은 셋이다. **게이트 판별력**(§3.5·§5.2) · **승인
안전성**(§6.5) · **계층 기여**(§5.4.2)이며, 확증 점검이 아니라 설계 근거로 다루되 판정은 그대로
보고한다. 운용 효율·거절 유형별 신호·의미 도달성·교차언어 회수는 처음부터 확증에 포함하지 않았고
탐색적 분석으로만 보고한다.

누출 검사는 qrel 마스킹 후 남은 금지 간선 수가 0인지 네 층에서 자동으로 확인하며, 개발 분할
실측은 7개 시스템 run 전량에서 위반 0이다. 재현성 통제가 완전하지 않은 지점 둘은 §6.6에 있다.
...
자원은 하류 검색 태스크의 평가 자산을 갖추었으나 세 세대에 걸쳐 T-Box의 술어 델타가 발생하지
않았고 자격을 갖춘 자원 변경은 한 건뿐이었다(§3.2 · §5.1). 제2 자원은 그 반대로, 인접 릴리스마다
실제 T-Box 변경과 공식 폐기·이행 규칙을 갖추었으나 정답 집합과 후보 풀이 없어 검색 조건을 적용할
수 없다. 곧 두 자원은 게이트의 서로 다른 절반을 평가하며, 본 절이 이식하는 범위는 그 겹치는 절반인
형식 층과 교차 태스크 층이다. 형식 검증 네 층과 조건 T3, 결함주입 절차는 코드 변경 없이 프로파일
파일의 교체만으로 옮기고, 정답 집합과 후보 풀을 요구하는 조건 T1·T2는 제외한다. 따라서 이 자원에서
승인식은 완성되지 않으며 형식 층과 교차 태스크 층의 논리곱만을 별도 필드로 기록한다.
...
DP4의 등급을 변경하지 않으며 교차 태스크 감시 원리(DP3)만이 승격 후보이다(전문 S8).
...
T1이 거부하였다(§5.1). 둘째, 형식 검증과 주 태스크 성능 검사가 모두 놓친 교차 태스크 결함을 조건
...
제시 순서는 에피소드 번호가 아니라 발견의 무게를 따른다. 승인식이 실제 델타를 거부한 기록을 첫
절에 두고, 자원 내 어휘의 존재를 확인한 표현 감사를 셋째 절에 둔다. 각 절의 첫 문단은 그 절의
결론과 확증 지위를 함께 진술한다. 에피소드별 지위는 표 3에, 사전등록된 점검의 판정은 §4.5의 표 6에 있다. 그림 6은 다섯
에피소드가 §3.5 승인식의 어느 항을 검증하였고 그 판정이 무엇인지를 대응으로 제시하며, 본 장의 각
절은 그 대응의 한 행을 상술한다.
...
## 5.1 EP3 · 통제된 자원 교체 — 게이트의 실제 거부 사례

형식 검증 네 층을 전부 통과하고 자원 지표를 전부 향상시킨 변경을 성능 조건 T1이 거부하였다. 이는
§3.5의 승인식이 실제 델타를 거부한 기록이며, 형식 검증이 태스크 조건을 대신할 수 없음을 보인다.

본 절은 별도 사전등록 아래의 판정이고 자원 스냅샷도 교정 후 세대이다. 따라서 §5.4의 확증 판정은
이 절로 변경되지 않으며, 승인 안전성에 대한 판정은 §6.5에 있다.
...
클래스 103 불변). 비교군은 둘이다. O 군은 교정 전 자원 번들을, O′ 군은 교정 후 자원 번들을 동일한 파이프라인에
투입한 실행이다. 여기서 자원 번들은 온톨로지·표면형 사전·개념 매핑으로 구성된다. 텍스트–개념
...
199로 늘었고 신규 링크 128,875건이 생성되었다. 곧 문서집합·코드·검색 설정·가중치·분할·봉인
qrel을 전부 동결한 파이프라인에서 자원 번들만 교체한 두 조건이 최초로 성립하였다. <!-- sig-history -->
...
이 비교가 식별하는 것은 T-Box 단독의 효과가 아니다. 동결된 파이프라인에서 자원 번들을 교체하였을
때의 질의 평균 회수 변화이다. 문서집합·코드·파라미터·가중치·분할·봉인 qrel을 고정하였으므로,
관측된 차이는 파이프라인의 차이나 평가 표본의 차이로 설명되지 않는다. 다만 자원 번들 내부의 어느
구성요소가 저하를 발생시켰는지는 이 비교만으로 분리되지 않는다.

본 연구는 이 판정을 거짓 거부로 읽을 가능성을 먼저 검토한다. 점수식의 결함이 저하를
발생시켰을 가능성은 배제되지 않는다. 그러나 승인식이 거부한 것은 자원 그 자체가 아니라, 그
자원을 그 파이프라인에 투입한 배포이다. 릴리스 승인의 단위가 배포이므로 거부의 대상과 판정의
대상은 일치한다. 실제로 온톨로지 단독 구성은 같은 교체에서 27% 향상되었다. 곧 이 결과는
온톨로지 보강이 무익하다는 증거가 아니다. 이는 자원 지표가 개선되고 형식 검증을 통과한 변경도
성능을 저하시킬 수 있다는 증거이다.

남은 두 가지를 함께 밝힌다. 첫째, 저하의 원인은 구분하지 못하였다. 문서당 개념이 2.4배로
증가하면 비가중 Jaccard의 분모가 커지고, 고빈도 일반 개념(`식각` 6,974문서 · `챔버` 6,462)이
변별력 없는 동점 블록을 형성한다. 그러나 본 실험은 이것이 자원의 결함인지 점수식의 결함인지
구분하지 못하며, 구분에는 문서빈도 가중이 필요하고 그것은 재측정이 아니라 새로운 방법이다.
둘째, 본 실행은 우월성의 재확증이 아니라 승인 규칙의 적용이다(§4.5).

본 사례가 운영에 대하여 지시하는 바는 하나로 한정된다. 이 1건에서 태스크 조건이 없었다면 family
Recall@100 기준 0.0293이 저하된 자원 번들이 릴리스되었을 것이다. 형식 검증만으로 승인하는
절차에서 이 변경은 승인되었을 것이다. 이 진술의 범위는 자격을 갖춘 변경 1건이며, 차단된 사고의
빈도나 일반적인 예방 효과를 지시하지 않는다.
...
## 5.3 EP1 · 표현 감사 — 세 태스크 어휘의 자원 내 존재
...
95.3%이다. 도메인 의미 관계로 연결되는 비율은 54.6–70.5%이며 분류코드까지 포함해야 95.3%에
도달한다(§3.3). 이 도달성은 언어에 따라서도 다르다. 후보 문서의 개념링크 보유율은 한국어 99.2% ·
영어 69.6% · 일본어 0%이고 분류 보유율만 세 언어 모두 100%이다. 곧 언어중립 개념 IRI는 T-Box
수준의 속성이며, A-Box 수준에서는 비한국어 문서에 개념이 적게 결합되어 있다. 이 비대칭이 §5.4.3을
해석하는 전제가 된다.
...
본 절의 값이 지지하는 주장의 범위는 셋으로 한정된다. CQ 통과는 질의 경로와 비공집합 응답의 존재를
지시할 뿐 세 태스크의 정확도를 검증하지 않는다. 공정 링크의 확장으로 한 CQ의 후보가 8건에서
90건으로 증가하였다는 이전 세대의 보고(S5)는 후보 생성 능력의 값이며 순위 품질의 값이 아니다.
그리고 G0·G1·G2의 T-Box가 동일하고 통과율 변동은 A-Box 인스턴스가 채워진 정도의 결과이므로, 본
...
qrel 503)이다. 두 패널은 통합하거나 평균하지 않는다. 패널 B의 판정식·마진·가중치·검색 설정은 패널
A의 것을 승계하여 개봉 이전 커밋(`67568c8`)에 고정되어 있다. 두 번째 분할은 확실성 조건 다섯
가운데 첫째인 두 비중복 분할에서의 재현을 확인한다(절 말미).
...
기준선의 값은 확증 판정에 들어가지 않고, 각 패널의 첫 행은 비교 기준선이므로 Δ 열이 정의되지
않는다. 단독 구성 넷의 행과 지연시간을 포함한 전체 결과는 S5에 있다. 분류 단독과 개념 단독의 R@100은
B3보다 약 0.25 낮으므로, 이 결과는 온톨로지를 텍스트 순위와 결합하는 방식의 효과를 보여준다.
...
**정답 불완전성에 대한 점검.** §4.5의 두 점검에서 상위 100위의 미판정 문서는 두 구성이
유사하였다. 해외 대응 출원의 심사관 인용을 병합하여 정답지를 넓힌 뒤에도 차이는
유지되었다(+0.0534 → +0.0593 · *p* = .008 → .003). 다만 취약성은 해소되지 않는다. 신뢰구간 하한을
0에 닿게 하는 최소 적대적 추가는 패널 A에서 4건, 패널 B에서 3건이다. 잔여 취약성은 표본 판정으로만
해소되며, 두 점검의 전문은 S5에 해소 명세는 §6.5에 있다.

**두 번째 분할의 표본 성질.** 패널 B의 질의는 온톨로지 신호가 희박하다. 문서당 개념이 2.909 대
1.105이고 200질의 중 83건에서 온톨로지 항이 구조적으로 0이다. 효과 축소는 이 성질과 방향이
일치한다. 이 성질은 개봉 이전 사전등록 문서에 기술되어 있고, 임계나 지표를 그 근거로 변경하지
않았다(전문 S5).
...
자원의 한계도 있다. 상류 원본 1,000건 중 진보성이 400건이고 신규성이 14건이며 신규성 단독 거절은
0건이다. 따라서 사전 예상이었던 신규성 대 진보성의 대비는 본 자원에서 검정할 수 없다. 교차언어 두
행도 단독으로 해석할 수 없다(§5.4.3).
...
정답 언어별 회수 분해와 검토 건수 지표 넷의 전체 표, 깊이별 회수 곡선은 전문을
[S5](../supplementary/S5-submission-full-v2.md)로 이관하였다.

같은 진단에서 온톨로지 구성의 상대적 우위 셋이 관측된다. 첫째, Ontology-only 구성의 영어 정답
회수는 0.109(14/128)로 전 구성 가운데 가장 높다(Text Hybrid 0.047 · 어휘 단독 0.000). 언어중립
개념 식별자가 어휘 장벽을 넘는 경로로 작동한 관측이다. 둘째, 일본어 후보 문서의 개념링크 보유율이
0.0%이므로 일본어 회수 0은 순위 산출의 실패가 아니라 자원 결손의 귀결이다. 셋째, 텍스트 기준선의
후보 풀이 담는 정답은 289/479(60.3%)이고 개념 단독 구성이 그 풀 밖에서 회수한 정답 45건을 더하면
상한은 334/479(69.7%)로 상승한다.
...
모델에 0건이었기 때문이다. 판정이 가능한 12건에서 T3의 단독 검출은 0건이고 불일치 쌍도 0건이다. 따라서 사전등록이 미리
기술한 검정력 한계가 그대로 발동한다(단측 McNemar *p* = 1.0000). 이 0을 검출 실패로 읽지 않는
근거는 둘이다. 첫째, 역량질문이 대상 인스턴스에서 실제로 행을 산출하는 범위를
**관찰면(observable surface)** 이라 한다. 홀드아웃 모델의 관찰면은 역량질문 15개 가운데 7개였고,
행이 0인 질문은 어떤 결함이 들어와도 회귀를 보일 수 없다. 개발 모델에서는 15개 전부가 행을
산출하였으므로, 이 결손은 질문의 결함이 아니라 홀드아웃 인스턴스의 충전 범위이다. 사전등록은 이
경우를 홀드아웃 미충족으로 보고하도록 미리 지시하였다. 둘째, 21건 전량에서 형식 검증 네 층과 교차
태스크 조건이 모두 미검출이었다. 곧 이 표본은 조건 T3을 다른 층과 비교할 수 있는 표본이 아니었다.
원인은 술어의 방향이다. 개발 모델은 포함관계를 `brick:hasPart`로 표현하여 주입 후보가 90건이었고
결함도 검출되었다. 반면 홀드아웃 모델은 같은 관계를 `brick:isPartOf`로 표현한다. 곧 결함 명세는
대상 자원의 모델링 관습에 맞추어 재정의되어야 한다. 이 관측을 게이트가 교차 결함을 놓쳤다는
진술로 옮기지 않는다.
...
선언하므로 동결한 델타 규칙이 반복하여 발화한다. 이행 조건 둘이 서로 다른 결과를 내지 않은 이유는
폐기 246건 가운데 기계 적용이 가능한 별칭이 1건뿐이었기 때문이다.

운용 비용은 탐색적으로 기록한다. 그래프 55,887 트리플과 역량질문 15개의 조건에서 층별 평균
실행시간은 L1 124초 · L2 12.5초 · L3+T3 1.6초이다. 최대 상주 메모리는 595 MB이다. 이 값은 실행
가능성에 대한 진술이며 규모 확장성의 근거가 아니다. 결함 인스턴스 전량의 판정과 동결 목록은
...
**① 자원 층 → 검색 층 (사전등록된 승인 규칙의 적용).** 사전 지정한 자원 커버리지 지표가 모두
향상된 변경을 동일 파이프라인에 교체 투입하였다. 그 변경에서 문서당 개념은 2.4배로 증가하였다. 그 결과 교체 대상
구성(P1)의 회수가 저하되었다(−0.0293 · 95% CI [−0.0542, −0.0053]). 형식 검증 L0–L3를 전부 통과한
그 변경을 성능 조건 T1이 거부하였다(§5.1 · 개발 분할의 사례는 S5).

**② 검색 층 → 운용 단위 (탐색적 기술통계).** 주 지표에서 유의하게 개선된 이득을 검토 건수 단위로
환산하면 소멸한다(감소율 중앙값 0.0% · 승·무·패 62/16/64 · §5.4.3).

**③ 검색 층 → 생성 층 (판정 1회).** 검색 구성만 교체하고 생성기를 고정한 조건에서 두 차례
측정하였다. 온톨로지를 포함한 구성은 두 평가 모두 인용 정확도의 점추정에서 앞섰다. 그러나 마진을
동결하고 수행한 판정은 실패하였으며(§3.5.1 · §6.5), 그 원인이 전달의 부재인지 검정력의 부족인지는
구분하지 못하였다.
...
층의 결과를 측정해야 한다는 것이다. 같은 구분에서 CQ는 정량 평가가 불가능한 태스크의 기능 보존을
검사하는 회귀 스위트라는 역할을 얻는다(§2.4 · T3).
...
여기에서 두 가지 수정된 주장이 도출된다. 재순위화로 결합된 온톨로지의 가치는 어휘 불일치의
해소에 있지 않다. 그 가치는 텍스트가 이미 회수한 후보 가운데 같은 기술 개념을 공유하는 문서를
검토 깊이 안으로 상승시키는 데 있다. 또한 지식 계층의 정밀도는 자동으로 검색 이득이 되지 않는다.
청구항 수준 의미가 순위에 기여하려면 평가 목표와 같은 단위의 매칭 함수가 필요하다. 이 상한은
교차언어 축에서 가장 크게 관측되었다(§5.4.3).

## 6.3 태스크 결합과 교차 태스크 의존성

한 태스크를 위하여 설계한 계층이 다른 태스크의 성능에 관여할 수 있음이 절제에서 관측되었다.
판정은 확정적이되 원인은 구분하지 못하였으며, 관측은 하나의 확증 분할과 이 릴리스의 모듈 배치에
조건부이다.

음성 대조군은 검색과 이론적으로 무관하도록 설계한 전문가 매칭 전용 계층이다. 이 계층을 제거하자
검색 성능이 유의하게 저하되었다(제거 손실 +0.0316 · *p* = .002 · 절제 8개 가운데 Holm 보정 후
유일한 유의 결과). 그 훼손은 형식 검증 L0–L3에서는 관측되지 않았다. 사전에 명시한 두 갈래는
특이성의 확립과 결합의 관측이었으며 관측된 것은 후자이다. 관측된 것은 영향의 존재이고 그 경로는 본
실험이 답하지 않는다. 개봉 이후의 자원 감사에서 보다 단순한 기제 둘이 실측되었다. 두 기제를
구분한 실험이 없으므로 원인은 확정하지 않으며, 근거는 S5에 예측은 §6.5에 있다. 두 번째 확증
분할에서 동일 절제의 값은 정확히 0.0000이므로 이 관측은 재현되지 않았다(§5.4.2). 두 표본이 갈린
원인도 구분하지 못하였다.
...
취한 규범적 선택이다(§3.5). 그러므로 각 판정이 전환점에서 얼마나 떨어져 있는가를 표 11에
보고한다. T1의 거부는 여유가 크다. 비열등 허용 폭이 지금의 2.7배로 넓어져야 판정이
...
임계는 §3.5에서 동결한 값 그대로이며, 표가 보고하는 것은 판정과 전환점의 거리이다.

## 6.4 설계원리 — 코어 넷과 적용 경계

본 절은 앞 절까지의 관측 가운데 무엇을 이전할 수 있는가를 다룬다. 관측을 설계원리로 지칭하면
주장의 범위가 산출물의 경계를 넘으므로, 승격 기준은 결과 확인 이전에 규정되어야 한다. 본 연구가 결과 확인 이전에 동결한 기준은 다섯이다. 근거가 실행된 코드의 출력일 것, 경쟁 설명의
배제 여부가 명시될 것, 산출물 밖으로 이전 가능한 형태로 진술될 것이 앞의 셋이다. 나머지 둘은 반증
조건을 함께 기술할 것과 설계 시점·확인 시점을 각각 기록할 것이다. 하나라도 미달하면 "제안"으로
분류한다. 다섯을 충족한 원리는 마지막 조건에 따라 설계가 선행한 "사전 설계 · 실증 지지"와 확인이
선행한 "경험적 지지"로 나눈다. 제안과 경험적 지지는 실패가 아니라 근거 수준에 상응하는 등급이며,
기준의 전문은 S5에 있다.

도출된 원리는 일곱이다. 아래 표의 **코어 원리** 넷은 승인 게이트의 핵심을 구성하고, 절 말미의
**범위 원리** 셋은 적용 경계를 규정한다. 이 구분은 근거 등급이 아니라 적용 위치의 구분이다.
...
표의 설계 시점 열은 사후 역산을 방지하는 장치이다. DP2·DP3은 게이트를 설계하는 시점에 이미 그
형태로 기술되어 있었고, 나머지 둘은 관측이 선행하였다. 관측이 선행한 원리를 처음부터 그렇게
설계하였다고 기술하는 것이 설계과학 연구에서의 사후 조정이다. DP1과 DP4의 등급이 경험적 지지에
머무르는 이유는 성립한 자원 델타가 1건이고 승인된 변경의 사후 안전성을 검정하지 못하였기
때문이다(§6.5).

DP1과 DP4는 종류가 다른 지침이다. DP1은 지표를 구분하는 방식에 관한 것으로, 이 배치가 없었다면
§5.1의 변경은 자원 지표의 전량 향상과 형식 검증의 전량 통과를 근거로 승인되었을 것이다. DP4는
평가의 설계에 관한 것이며 방법론적 요구사항의 성격을 가진다. 자원 변경의 효과를 판별하려면
문서집합·모델·가중치를 고정한 채 자원 번들만 교체해야 한다. 본 연구에서 그 조건이 성립하기까지
T-Box가 변경되지 않은 세 세대를 지나야 하였다(§3.2).
DP3의 근거는 제2 공학 온톨로지의 판정으로 늘어나지 않았다. 판정이 가능한 결함 12건에서 T3의 단독
검출이 0건이었기 때문이다(§5.5). 다만 이 0은 아래 도전 조건이 성립한 결과가 아니다. 그 표본에서는 형식 검증 네 층과 교차 태스크
조건이 모두 미검출이어서 검출률 비교의 대상이 성립하지 않았다. 따라서 이 에피소드는 도전 조건을
판정하지 못하였고 등급도 변경하지 않는다. 0건의 분모는 21건이 아니라 12건이며, 나머지 9건은
결함이 주입되지 않아 판정에 이르지 못하였다.

각 원리에는 그 필요성을 약화시킬 수 있는 **경험적 도전 조건**을 함께 둔다. DP1은 자원 지표와
태스크 지표가 여러 세대에서 일관되게 함께 움직일 때 약화된다. DP2는 형식 검증만으로 하류 비회귀가
예측되고 태스크 조건이 승인 결정을 한 번도 바꾸지 않을 때 약화된다. DP4는 문서집합까지 바뀐
비교에서도 자원 번들의 효과를 분리할 수 있을 때 약화된다. DP3은 주 태스크 검사의 검출률이 T3과
같고 T3의 단독 검출이 0일 때 약화된다. 다만 제2 도메인의 12건은 층 사이의 불일치 쌍이 0이어서 이
조건을 판정하지 못하였다.

**적용 경계.** 범위 원리 셋은 위 넷의 적용 범위를 규정하며 등급은 앞의 기준이 정한 그대로이다.
**DP5 · 후보생성 분리**는 재순위화가 후보 풀 밖의 문헌을 회수할 수 없으므로 후보 생성과 구분하여
평가해야 한다는 원리이며 등급은 경험적 지지이다. 근거는 검토 깊이에 상한을 두지 않았을 때 세
시스템의 미도달 질의 비율이 완전히 동일하다는 관측이다(정답 절반 26.8% · 완전 회수 55.1% ·
§5.4.3). 재순위화만으로 그 비율이 감소하면 이 원리의 필요성이 약해진다. **DP6 · 전달 검증**은
검색 지표의 개선을 검토 효율이나 생성 품질로 자동 일반화하지 않는다는 원리이며 등급은 제안이다.
판정 자체는 이 원리를 지지하는 방향이다. 다만 실패의 원인이 전달의 부재인지 검정력의 부족인지는
구분하지 못하였으며(기준 ② 미달 · 실패의 폭은 표 11), 구분에는 새 사전등록이 필요하다. 검색
지표의 개선이 검토 건수나 생성 품질로 일관되게 전달되면 이 원리의 필요성이 약해진다. 세 원리의
근거 표는 S5에 있다.

**DP7 · 이식 계층 분리(port-layer separation)** 는 이전되는 것과 재접지가 필요한 것을 가르는
원리이다. 승인 게이트를 다른 자원으로 옮길 때 층 구조와 판정식, 실행으로 이루어진 기제는
이전된다. 반면 델타 제약과 결함 정의, 역량질문으로 이루어진 명세는 대상 자원의 표현 관습과
인스턴스 관찰면에 재접지하여야 한다. 등급은 경험적 지지이며 확인이 설계보다 앞섰다. 근거는 제2
자원에서 관측된 세 자리이다. 동결한 델타 제약은 정의역과 치역을 스키마 어휘로 선언할 것을 요구하나
대상 자원은 제약 언어로 선언한다. 그 결과 릴리스 계보 10판정 가운데 8판정이 이 규칙에서
미충족이었다. 동결한 결함 정의는 포함관계를 한 방향의 술어로 기술하나 홀드아웃 모델은 반대 방향을
사용하므로 21건 가운데 9건이 주입되지 않았다. 역량질문 15개 가운데 홀드아웃에서 행을 산출한 것은
7개였다(§5.5). 이식 사례가 1건이므로 근거의 폭은 그만큼으로 한정한다. 표현 관습과 인스턴스 구성이
다른 자원에 명세를 그대로 옮겼는데도 검출력이 유지되면 이 원리의 필요성이 약해진다.

이 원리는 앞의 결과를 다시 읽는 방식을 바꾼다. 제2 자원의 검출 0을 게이트의 실패로 읽으면 원리와
그 구현이 구분되지 않는다. 실측이 가리키는 것은 기제와 명세가 서로 다른 것에 의존한다는
사실이다(§6.1).

## 6.5 한계 · 경쟁 설명 · 결손의 명세

**승인 안전성.** 승인된 변경의 안전성은 미검정이다. 비교 자격은 T-Box 델타와 개념층 델타에만
있는데, G0·G1·G2의 T-Box가 동일하여 자격을 갖춘 변경이 계보에 없었다(§3.2). 자격이 최초로 성립한
§5.1의 판정은 승인 거부였다. 곧 거부하는 능력은 1회 관측되었고 승인한 변경의 사후 안전성은
관측되지 않았다.

**구성 타당도.** 정답이 심사관 인용으로 규정된 집합이므로 측정된 값은 그 집합의 회수이다(§2.1).
전문가 관련성 판정을 수행하지 않아 상위 미인용 후보의 실제 관련성은 미관측이다. 이 미관측은 절대
성능의 진술을 제약한다. 짝지은 차이는 동일한 정답을 적용하므로 영향을 덜 받으나 완화는
부분적이다(표 13 ⑧).
...
**언어와 타당도 일반.** 질의는 전량 한국어이고 후보 코퍼스는 다국어이므로(한국어 39,246 · 영어
1,189 · 일본어 117), 본 논문이 보고하는 Recall은 한국어 하위풀에 대한 회수에 가깝다. 다른
관할·언어·분야로의 일반화는 검증되지 않았다(표 13 ⑥).
...
전달을 확증하지 못하였다는 것이다. 전달의 부재인지 검정력의 부족인지는 구분하지 못하였다. 이
...
**경쟁 설명.** §5.1의 관측, 곧 자원을 2.4배 개선하였음에도 검색 성능이 저하된 현상에는 경쟁 설명
...
**결손의 명세.** 아래 표는 검증 강도와 일반화를 제약하는 결손 아홉과 해소 명세이다.

**표 13. 검증 강도와 일반화 가능성의 결손 아홉 — 해소 방법은 전부 새 사전등록의 대상이다.**

| | 결손 | 현 상태 (보고 위치) | 해소하는 측정 |
|---|---|---|---|
| ① | 실제 자원 변경의 거부 사례가 1회 | 통제된 자원 교체 1회에서 T1 미충족에 따른 승인 거부 (§5.1) | 자격 있는 델타 3–5건을 순차 투입하고 판정의 분포를 보고 |
...
| ⑤ | 승인 게이트를 완전히 평가할 수 있는 자원의 부재 | **부분 해소** — 형식 층과 교차 태스크 층은 제2 공학 온톨로지에서 동일 절차로 판정하였다 (§4.6 · §5.5). 게이트 전체의 평가는 릴리스 델타 계보와 하류 태스크 벤치마크를 동시에 갖춘 자원을 요구하나, 본 연구의 자원은 뒤의 것만 제2 자원은 앞의 것만 갖추었다 | 두 조건을 동시에 갖춘 자원의 확보, 또는 제2 자원의 태스크 벤치마크를 구축한 뒤 T1·T2를 적용 |
...
| ⑧ | 전문가 관련성 판정 미수행 | 프로토콜은 동결하였으나 판정은 수행하지 않았다 (§4.4). 취약도는 판정 전환 최소 건수로 계량하고 외생 라벨 병합으로 **부분 축소**하였다 (§5.4.1) | 상위 미인용 후보의 **표적 표본**에 대한 2인 가림 독립 판정과 κ 보고 |
| ⑨ | 결함 명세의 자원 간 이전 | 제2 도메인의 홀드아웃 모델에서 포함관계 역전 결함의 주입 후보가 0건이어서 21건 중 9건이 판정에 이르지 못하였다 (§5.5) | 대상 자원의 술어 방향을 실측하고 그 관습에 맞추어 결함 명세를 재정의한 뒤 새 사전등록으로 재판정 |
...
- **평가 자산** — 질의·qrel 식별자와 분할 경계 · 동결 임계 전량(\(\epsilon\)=0.02 ·
  ...
  전달 평가 절차(모델 ID·컨텍스트 개수·온도·반복·프롬프트 원문과 sha256) · 제2 도메인 이식 자산.
- **평가 하네스** — 검색 구성 일곱의 구현과 게이트 조건, 채점과 통계, 그림 생성 코드가
  `benchmark/` 에 수록된다.
- **미공개와 사유** — KIPRIS 원문은 학술이용 조건상 재배포할 수 없고 회사 기밀 계층은 비식별
  변조본만 존재한다. 원문을 포함하는 계층은 재인출로 대체되며, 재인출로 특허 A-Box 를 재생성하면
  트리플이 0.009 % 오차로 일치한다.
...
공개 자산 전량의 파일별 sha256은 릴리스의 `provenance/PROVENANCE.json` 에 등재되어 있다. 데이터셋은
이후에도 개선되므로, 재현 시에는 최신 상태가 아니라 위 릴리스와 그 해시를 대조해야 한다.

재현 가능성은 자산에 따라 세 등급으로 나뉜다. 공개 파일만으로 동일한 산출을 얻는 등급, 식별자로
원문을 재인출하여 근사적으로 재현되는 등급, 그리고 감사만 가능한 등급이다. 공개 저장소의 실측에서
둘째 경로의 CQ 통과율은 27/31이다. 셋째 등급에는 청구항 한정요소 층과 그 층에 의존하는 CQ 네
건이 속하며, 분해 입력이 원문이므로 키를 발급받아도 재생성되지 않는다. 본문이 밝힌 결손 둘도 셋째
등급이고 동결된 판정을 변경하지 않으나 완전한 독립 재실행을 제한한다(§4.5). 원고가 보고하는
수치는 동결 세대의 값이고 릴리스는 이후의 상류 교정을 반영한 현행 세대이므로, 자산별 등급과 두
세대의 수치 대조는 [S7](../supplementary/S7-release-crosswalk.md)에 있다.
...
원리는 게이트를 다른 자원으로 옮길 때 기제는 이전되지만 명세는 재접지를 요구한다는 것이며, 제2
공학 온톨로지에 대한 판정에서 관측되었다.

**기여와 한계.** 기여는 검증 자산을 포함한 SDKB와 변경 승인 게이트, 그리고 이전 가능한
설계원리이다. 다만 태스크 성능을 측정한 대상은 단일 도메인이고 실제 변경의 거부 사례는 1회이며,
저하 원인과 생성 단계 전달 여부도 확정하지 못하였다. 제2 자원에서 판정한 범위는 형식 층과 교차
태스크 층이고 이식 사례는 1건이다. 자유 텍스트 질의를 개념에 연결하는 단계와 §6.5의 결손을
