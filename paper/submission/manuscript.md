...
| RQ | 연구질문 — 설계 질문의 라벨 (§1) |
| L1–L3 | 본 연구의 교훈 — 후속 연구가 검정할 가설로 진술한 설계 교훈 (§6.3) |
...
| 설계지식 | 교훈 L1–L3 과 후속 가설 둘 | §6.3 · §6.4 |
...
층에 관한 교훈의 근거를 늘리지 않으며, 교차 태스크 감시(§6.3 교훈 L2)만이 근거 확대의
대상이다(전문 [S8](../supplementary/S8-second-domain-port.md)).
...
**교훈 L1 · 한 층 아래 승인.** 자원 변경은 자원 지표가 아니라 다음 사용 층의 비회귀 결과로
...
**교훈 L2 · 교차 태스크 감시.** 공유 T-Box에서는 주 태스크의 성능만으로 변경을 승인하지 않는다.
...
**교훈 L3 · 후보생성 분리.** 재순위화는 후보 풀 밖의 문헌을 회수할 수 없으므로 후보 생성과
