...
결과는 §3이 정의한 네 에피소드의 순서로 제시한다. 자원에 무엇이 존재하는가(EP1), 게이트가
결함을 검출하는가(EP2), 게이트가 실제로 무엇을 거부하였는가(EP3), 검색 효용의 범위는
어디까지인가(EP4)의 순서이다. 이 순서는 판정의 전제가 되는 항목을 앞에 배치한 결과이다.
에피소드마다 지위 또한 상이하다. EP1은 관측 사실이고, EP4는 확증 점검 둘을 담당하며, EP3은 별도
사전등록 아래의 판정이다. EP2는 규칙을 동결한 채 홀드아웃으로 확증한 실행이나, 그 점검은 §4.5의
확증 목록에서 설계 근거로 강등되어 있다.
...
전량 31개 기준 G0는 30개이다(§3.4 표 3). 그러나 이 결과는 질의 경로와 비공집합 응답의 존재를 의미할
뿐, 세 태스크의 정확도가 검증되었음을 의미하지는 않는다.
...
8건에서 90건으로 증가하였다는 보고에서 그중 몇 건이 알려진 인용인지는 측정되지 않았다. 한정요소
자원으로는 Claim 586,567개·ClaimFeature 1,289,512개가 존재하고 판단 연결
표본의 도달성은 402/584(68.8%)이다. 이는 청구항 수준 평가의 실행 가능성을 보일 뿐, 질의 1,000건
전체가 동일한 완전도를 갖는다는 의미는 아니다.

교차 태스크 CQ 통과율은 저하되지 않았으며 누적 waiver는 0회이다. 다만 실제 그래프 세대가 둘뿐이므로
추이라 부를 수 있는 이력은 확보되지 않았으며, 표를 채우기 위해 세대를 생성하지는 않는다. G0·G1·G2
사이의 통과율 변동도 온톨로지의 개선이 아니라 A-Box 충전의 결과이다(세 T-Box가 동일하다).
그러므로 본 절의 수치는 세대 안전성의 증거가 아니다(§6.5).

## 5.2 EP2 · 게이트 판별력 — 홀드아웃 확증

층을 분리한 이후의 게이트는 아직 판정한 적 없는 결함에서 사전 지정한 세 조건을 모두 충족하였다.

게이트의 판별력은 §4.5의 확증 점검 셋에 포함되지 않으며 그 지위는 설계 근거이다. 다만 아래의
홀드아웃 재판정은 규칙을 동결한 채 수행한 확증 실행이다. 최초에 사전등록한 형태는 다른 태스크를
...
층 분리 이후의 판별력은 아직 판정한 적 없는 결함으로 확증하였다. 규칙을 변경하지 않은 상태에서
...
고정한 데 의존하며, τ=0.10에서는 T3 단독 검출이 4/45로 감소하여 성립하지 않는다(§6.5). ② 공유 계층
역전은 이번에도 0/9이며 게이트의 완전성은 확증되지 않았다. ③ T3의 특이성은 아직 검정되지 않았다.
④ 형식층 L2에는 검출 표면이 사실상 존재하지 않는다. T-Box에 서로소·카디널리티 제약이 없어 타입
모순의 주입이 모순으로 성립하지 않기 때문이다.

## 5.3 EP3 · 통제된 자원 교체 — 게이트의 실제 거부 사례

형식 검증 네 층을 전부 통과하고 자원 지표를 전부 향상시킨 변경을 성능 조건 T1이 거부하였다.

> **주의.** 본 절은 §5.4와 다른 실험이다. 자원 스냅샷이 상이하며(교정 후) 별도의 사전등록 아래
> 수행되었고, 본 논문의 어떤 확증 판정도 변경하지 않는다. 이를 보고하는 이유는 §3.5의 승인식이
> 델타를 거부한 유일한 기록이기 때문이며, 승인 안전성에 대한 판정은 §6.5에 있다.
...
구분하려면 문서빈도 가중을 도입해야 하며, 그것은 재측정이 아니라 새로운 방법에 해당한다. ③ 본 실행은 우월성의 재확증이 아니다. 이미
...
사전등록된 복합 기준의 동시 충족은 두 분할에서 확인되지 않았고, 깊은 회수의 개선은 두 분할에서
반복 관측되었다.
...
표의 모든 수치는 분석 CLI의 출력이며 조건은 전 시스템과 두 패널에서 동일하다. 후보 마스크(시점
유효 · 자기 및 동일 패밀리 배제)를 적용한 뒤 상위 1,000건에서 절단한다. 이어 family 수준으로
집계하며, 알려진 양성이 하나 이상인 질의만 매크로 평균한다. 패널 B의 판정식·마진·가중치·검색 설정은 패널
A의 것을 승계하였으며 개봉 이전 커밋(`67568c8`)에 고정되어 있다.
...
불완전 정답 아래의 강건성은 §4.5의 두 점검으로 확인하였다. 상위 100위의 미판정 문서 조성은
언어·공개 연도·개념 수의 세 축에서 두 구성이 유사하였다. 분류 축은 패널 A에서 상이하였다.
B3의 미판정 집합은 반도체 소자 분류(H10)가 0.332로 최빈이다. 반면 P1은 표면 처리 분류(C23)가
0.320으로 최빈이며, 패널 B에서는 두 구성의 분류 조성이 유사하였다. 분류 축은 잔여 위협으로
남으며 이 관측으로 기존 판정을 수정하지 않는다. 판정 전복 문턱은 작게 산출되었다. 패널
A에서 신뢰구간 하한을 0에 닿게 하는 최소 적대적 추가는 4건이고 점추정 기준으로는 17건이며,
패널 B에서는 각각 3건과 10건이다. 상대량은 적대적 추가가 가능한 후보 수 대비 0.003 이하이다.
정답이 한 건뿐인 질의가 약 24%를 차지하므로 그 질의에서는 한 건의 추가가 회수를 전부 상쇄한다.
본 논문의 짝지은 차이는 미판정 문헌의 소수 조성에 취약하며, 이 취약성은 표본 판정 없이는
해소되지 않는다(§6.5). 축별 분포와 전량 수치는
[S5](../supplementary/S5-submission-full-v2.md)에 있다.

이 취약성을 사람 판정 없이 한 번 더 점검하였다. 동일 발명의 해외 대응 출원에 부여된 심사관 인용은
본 연구의 어떤 시스템보다 먼저 그와 무관하게 생성되었으므로 외생적 라벨이다. 이를 병합하여 정답지를
넓힌 뒤 동결된 순위를 다시 채점한 결과, 짝지은 차이는 유지되었다(+0.0534 → +0.0593 · *p* = .008 →
.003). 다만 증분이 13쌍이므로 정답 불완전성의 영향이 없음이 입증된 것은 아니다. 부수적으로, 도달한
408쌍 가운데 297쌍이 이미 국내 심사관 인용과 일치하였다. 서로 다른 관할의 심사관이 독립적으로 같은
선행 문헌에 도달하였다는 뜻이며, 본 정답지가 한 심사관의 자의적 선택이 아님을 시사한다. 전량은
[S5](../supplementary/S5-submission-full-v2.md)에 있다.
...
0이므로 제안 시스템이 텍스트 기준선과 동일한 순위를 산출한다. 이 사실은 개봉 이전 사전등록 문서에
기술되어 있으며, 본 연구는 이를 근거로 임계나 지표를 변경하지 않았다. 패널 B의 효과 축소는 이
표본 성질과 방향이 일치한다.
...
환산하면 소멸한다. 본 절의 값은 전부 탐색적 기술통계이며 결론의 주장에 포함하지 않는다.
...
본 진단의 비교 범위도 함께 밝힌다. 대비된 것은 본 파이프라인의 구성이며 다국어에 특화된 밀집
기준선은 포함되어 있지 않다(§4.3). 따라서 이 결과는 교차언어 검색 방법의 상한이 아니라 본
파이프라인의 상한을 지시한다. 후보 풀의 편향도 함께 보고한다. 영어 문서의 72.5%, 일본어 문서의
70.9%가 정답이어서 외국어 하위풀에는 방해 문서가 희소하다. 본 진단의 관측은 회수가 0에 근접한다는
방향이므로 이 편향의 영향을 받지 않는다.
...
이 구분은 표현 범위와 검증 깊이의 구분과도 이어진다. SDKB가 세 태스크의 클래스와 관계를 T-Box에
포함한다는 진술은 표현 범위에 관한 주장이다. 반면 선행기술 검색에서 Recall@K를 검정하는 것은 한
태스크의 검증 깊이를 측정하는 주장이다(§2.4). 이 구분은 일부 CQ의 통과만으로 모든 태스크의 효용을
주장하는 과잉 일반화를 차단하며, CQ와 검색 평가는 서로를 대체하지 않는다. 동시에 CQ는 정량 평가가
...
가운데 Holm 보정 후 유일한 유의 결과). 사전에 명시한 두 갈래는 특이성의 확립과 결합의 관측이었으며,
관측된 것은 후자이다. 즉 전문가 매칭 뷰를 대상으로 한 델타가 선행기술 검색의 질의 경로에 영향을 줄
수 있다. 그 훼손은 형식 검증에서도 게이트 태스크의 성능 검사에서도 관측되지 않는다.
...
또한 두 번째 확증 분할에서 이 관측은 재현되지 않았다. 동일 절제의 값이 정확히 0.0000이었다(§5.4.2).
본 연구는 두 값을 평균하거나 어느 한쪽을 취소하지 않는다. 정확한 진술은 두 표본에서 결과가 갈렸다는
것이며, 갈린 이유는 미구분이다. 첫 분할의 유의한 악화가 우연이었을 수도 있고, 두 번째 분할의
질의에 해당 축의 개념이 부재하여 제거할 대상이 없었을 수도 있다.

이러한 갈림이 교차 태스크 조건의 근거를 무효화하지는 않는다. 그러나 근거의 무게는 이동한다. 근거
둘 가운데 하나가 여기에서 갈렸고, 나머지 하나는 유지된다. 형식 검증과 검색 성능 검사가 모두 놓친
결함을 T3만 단독으로 검출한 사례가 홀드아웃 45건 중 12건(단측 *p* = .0001)이라는 관측이다(§5.2).
따라서 교차 태스크 감시 원리의 근거도 결함주입을 앞에 두고 음성 대조군을 갈림과 함께 뒤에 둔다. 본
연구는 이 관측을 승격하지 않으며, 현재 등급은 **설계 진단**이다.
...
확인하기 전에 규정해 두지 않으면, 원리는 결과의 사후 요약으로 전락한다. 본 연구가 결과 확인
이전에 설정한 문턱은 다섯이다. ① 근거가 실행된 코드의 출력일 것. ② 경쟁 설명이 배제되었거나,
...
**재순위화와 매핑 단계의 부재.** 첫 번째 핵심 제약은 재순위화 방식에서 발생한다. 제안 순위 함수는
텍스트 기준선의 상위 1,000건을 재정렬할 뿐 후보 집합을 확대하지 않는다. 따라서 측정된 값은
온톨로지가 선행기술 검색을 개선하는 정도의 상한이 아니라 재순위화라는 결합 방식에서의 개선분이다.
