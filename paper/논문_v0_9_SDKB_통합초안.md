...
| **DP3** | **교차 태스크 감시** — 공유 T-Box 에서는 **주 태스크 성능만으로** 변경을 승인하지 않는다 | §6.4 — 검색과 무관하게 설계한 음성 대조군(A8) 제거가 검색을 −0.0316 악화 · §6.5 — 교차 결함을 T3 만 단독 검출(12/45 · 단측 *p*=.0001) | **§4.9 설계 시점**(T3) | 2026-07 | **확립** (경쟁 설명 §8.2 명시) |
...
| **DP6** | **전달 검증** — 검색 지표의 개선을 검토 효율이나 생성 답변 품질로 **자동 일반화하지 않는다** | §6.3a — Recall@100 이 +0.0534 인데 Candidate Reduction 중앙값 **0.0 %** · §6.8 — 생성 층 판독은 방향이 갈렸다(인용 정확도 우세 · 환각률 열세) | §4.9.1(T4 설계) | **미완** | **제안** |

**DP6 이 제안에 머무는 이유를 적는다.** 문턱 ②(경쟁 설명 배제)와 ⑤(확인 시점)가 미달이다 —
생성 층 판독은 개봉된 분할 위의 탐색이었고 유의성 검정을 사전등록하지 않았다(§6.8). 승격은
새 확증 분할에서 같은 계측기로 다시 잰 뒤에 **다시 판정**한다. **지금 올리지 않는다.**
...
## 8.3 아직 만들지 못한 산출물 — 실패 유형의 질적 분류

DP1·DP6 을 뒷받침할 정성 축이 하나 비어 있다. **온톨로지 팔이 순위를 악화시킨 질의를 코딩해
실패 유형을 만드는 작업**이며, 시드 사례는 이미 있다(§7.5 ③ — 공유 개념이 5개에서 9개로 늘었는데
같은 정답이 4위에서 67위로 밀린 질의).

**하지 않은 이유는 자원이 아니라 방법이다.** 프로토콜은 §5.5 가 이미 설계해 두었고 —
시스템명과 순위를 가린 **2인 독립 코딩** · Cohen's κ 와 합의율 보고 · κ<0.4 이면 본문에서
제외하고 부록으로만 — **두 번째 코더 없이 이 절차를 밟을 수 없다.** 한 사람이 코딩한 결과에
κ 를 붙일 수 없고, κ 없는 질적 코딩을 정량 부족의 대체물로 싣지 않는 것이 이 논문의 규율이다.

그래서 이 자리는 **비워 둔 채로 명시한다.** 채워지면 DRQ3 의 답이 한 겹 깊어지고, 채워지기
전까지 §8.1 의 DP 는 **정량 근거만으로** 서 있다.
