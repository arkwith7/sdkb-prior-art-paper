...
| **H3** (하이브리드 효과) | P0 또는 P1이 B3보다 R@100·nDCG@20 개선(보정 후 유의)이고, low-overlap 집단의 개선폭이 high-overlap보다 큼 | test(198q·family R@100): P1 Δ+0.0534, 95%CI[+0.0145,+0.0926], p=0.008 (유의); 개념단독 Δ+0.0584, p=0.002. 주 사전지정 P0★(concept+ipc)는 Δ+0.0319, p=0.181 (유의 미달 — IPC 가중 dev 과적합). **개선폭은 개념중첩 high 집단서 더 큼**(D3), low-overlap 조건부 예측과 반대. nDCG@20 `[실험 후 기입]` | **부분 지지** (P1 유의·주 P0★ 유의 미달·low-overlap 조건 반증) |
...
| -CPC/IPC (A1) | 198 | 479 | — | 0.4824 | +0.0025 | [−0.0259,+0.0300] |
| -ClaimFeature (A4) | 198 | 479 | — | 0.4779 | +0.0070 | [−0.0038,+0.0215] |
...
> §6.4 ablation 행: `제안법 R@100` = P1(full 0.4849)에서 해당 계층 제거 후 값 · `차이` = **제거 손실
> (full − ablated · 양수 = 계층 기여)** · 95% CI = 질의단위 paired bootstrap(10k·seed 20260726). test 198질의·
> family R@100. A8(음성 대조군)만 Holm(m=8) 유의 → §6.3 H5 기각·태스크 결합 발견. 신규성/진보성·
> low/high lexical overlap 행은 F11(문자 n-gram Jaccard) 사전등록 정의·거절유형 라벨 산출 후 채운다(후속).
