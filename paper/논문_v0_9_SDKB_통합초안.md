...
이 절은 현재 자원 구조와 관련연구로부터 도출한 **반증 가능한 예상**과 결과별 해석 규칙을 제시한다. 예상 3·5·6은 확증 분할에서 이미 검정되었으므로 각 항에 **관측** 문단을 두어 예상과 실측을 나란히 남긴다 — 예상을 사후에 고쳐 맞히지 않는다. 예상 3과 5는 **반증되었고**, 예상 6은 두 갈래 중 결합 발견 쪽으로 확정되었다. 예상 1·2·4·7은 미검정이며 그 사실을 각 항에 명시한다.
...
**질의·문서 번역 미적용.** 후보 코퍼스와 질의는 한국어이나 알려진 양성의 41%가 비한국어(영어 39%·일본어 2%)다. 본 실험 파이프라인에는 질의 번역 단계가 구현되어 있지 않으므로, 교차언어 집단에서 관측된 낮은 이득(§6.4)은 온톨로지의 언어 중립성 부족이 아니라 **번역 부재와 교란되어 있다**. 두 요인을 분리하려면 번역 단계를 추가한 별도 비교가 필요하다.
...
- **질의·문서 번역이 구현되어 있지 않다.** 알려진 양성의 41%가 비한국어이므로 교차언어 하위집단 결과는 번역 부재와 교란되어 있다(§9.1).
