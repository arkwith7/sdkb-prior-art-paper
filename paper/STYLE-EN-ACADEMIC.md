# 영문 학술 논문 문체 규격 v1 (AEI 투고용 · 2026-08-22 초안)
...
> (`paper/manuscript/en_source.md`)의 어체와 **한국어 → 영어 전환 규칙**을 정한다.
> 용어의 확정어는 [glossary.md](glossary.md)(용어 대장)가 정본이며, 이 문서는 그 대장을
> **문장 수준**으로 확장한다 — 낱말이 맞아도 구문이 다르면 주장 강도가 바뀌기 때문이다.
>
> **적용 범위는 한국어 규격과 같다 — "내용 보존 + 어체 전환".** 수치·판정·기여·장 구성은 이
> 규격으로 바꾸지 않는다. 표·그림·캡션은 `scripts/build_submission_en.py` 가 한국어 파생본에서
...
**S5 · 한국어 한 문단 = 영어 한 문단.** 번역 과정에서 문단을 쪼개거나 합치지 않는다. 문단 수가
같아야 `{{TABLE:n}}`·`{{FIGURE:n}}` 의 자리와 절 참조(§)가 한국어 파생본과 1:1 로 대응하고,
`submission_check` D9 가 두 원고에 같은 결과를 낸다.
...
1. **문단 대응** — 한국어 파생본과 영문 산문의 문단 수가 절마다 같은가(S5).
2. **수치 불변** — `build_submission_en.py` 가 표·그림에서 강제한다. 산문 안의 수치는 아래
   `numdiff` 와 같은 방식으로 한국어 산문과 대조한다(§번호·표/그림 번호·라벨 기호 제외).
