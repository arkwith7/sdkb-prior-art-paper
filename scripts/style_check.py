...
#
# **PLAN-088 에서 둘을 더했다.** 관통 예시가 합성 픽스처에서 데이터셋에 등재된 실사례로
# 바뀌었고(§3 예시 1 · §5.2 추적 대상), 그 지위는 기존 넷 가운데 어느 것도 아니다 —
# 합성이 아니고, 결과를 보고 고른 탐색 사례도 아니며(동결 규칙이 지정한다), 집계도 아니다.
# 등급을 늘리는 것은 구분을 푸는 것이 아니라 **없던 칸을 만드는 것**이다. `실사례` 는
# 인스턴스의 출처를, `절차 설명` 은 그 인스턴스 위에서 절차만 기술하는 자리를 가리킨다.
